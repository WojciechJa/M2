...
#ifdef ENABLE_EXTENDED_ITEMNAME_ON_GROUND
void CPythonItem::CreateItem(DWORD dwVirtualID, DWORD dwVirtualNumber, float x, float y, float z, int count, bool bDrop, long alSockets[ITEM_SOCKET_SLOT_MAX_NUM], TPlayerItemAttribute aAttrs[ITEM_ATTRIBUTE_SLOT_MAX_NUM])
#else
void CPythonItem::CreateItem(DWORD dwVirtualID, DWORD dwVirtualNumber, float x, float y, float z, int count, bool bDrop)
#endif
{
	//CItemManager& rkItemMgr=CItemManager::Instance();
	...
	switch (pItemData->GetType())
	{
		case CItemData::ITEM_TYPE_POLYMORPH:
		{
			...
			len += _snprintf(szItemName, sizeof(szItemName), "%s", c_szTmp);
			break;
		}
		...
		case CItemData::ITEM_TYPE_SKILLFORGET:
		{
			...
			if ((dwSkillVnum != 0) && CPythonSkill::Instance().GetSkillData(dwSkillVnum, &c_pSkillData))
				len += _snprintf(szItemName, sizeof(szItemName), "%s", c_pSkillData->GradeData[0].strName.c_str());
	...
	if (count > 1) {
		//if (!pItemData->GetType() == CItemData::ITEM_TYPE_ELK) {
		if (pItemData->GetType() == CItemData::ITEM_TYPE_ELK) {
			len += _snprintf(szItemName, sizeof(szItemName), "%s", pItemData->GetName());
		} else {
			len += _snprintf(szItemName + len, sizeof(szItemName) - len, (len > 0) ? " %s |cffc4c4c4(x%d)|r" : "%s |cffc4c4c4(x%d)|r", pItemData->GetName(), count);
		}
	}
	else {
		len += _snprintf(szItemName + len, sizeof(szItemName) - len, (len > 0) ? " %s" : "%s", pItemData->GetName());
	}
...
void CPythonItem::Create()
{
	...
	//Rare
	__RegisterEffect(DROP_ITEM_EFFECT_RARE, "d:/ymir work/realis_effects/dropeffect/rare_yellow.mse");

	//Epic
	__RegisterEffect(DROP_ITEM_EFFECT_EPIC, "d:/ymir work/realis_effects/dropeffect/rare_green.mse");

	//Legendary
	//__RegisterEffect(DROP_ITEM_EFFECT_LEGENARY, "d:/ymir work/realis_effects/dropeffect/rare_1.mse");
}
...
int CPythonItem::__AttachEffect(DWORD dwVnum, BYTE byType, BYTE bySubType)
{
	...
	if ((dwVnum >= 56000 && dwVnum <= 56040) ||
		dwVnum == 90037 || dwVnum == 90038 || dwVnum == 90039 || dwVnum == 90040)
	{
		return DROP_ITEM_EFFECT_EPIC;
	}
	...
	return DROP_ITEM_EFFECT_NORMAL;
}