...
bool CGuildMarkManager::GetMarkImageFilename(DWORD imgIdx, std::string & path) const
{
	...
	char buf[64];
	_snprintf(buf, sizeof(buf), "mark/%s_%lu.tga", m_pathPrefix.c_str(), imgIdx);
	path = buf;
	return true;
}
...
{
	char buf[64];
	_snprintf(buf, sizeof(buf), "mark/%s_index", m_pathPrefix.c_str());
	FILE * fp = fopen(buf, "r");
...
int main(int argc, char **argv)
{
	...
	{
		_snprintf(f, sizeof(f), "%lu.jpg", (random() % 5) + 1);
		//SaveMark(i, f);
		mgr->DeleteMark(i);
	}
	*/
