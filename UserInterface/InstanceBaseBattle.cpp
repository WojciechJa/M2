...
BOOL CInstanceBase::CheckAdvancing()
{
	...
	if (!__IsMainInstance() && !IsAttacking())
	{
		if (IsPC() && IsWalking())
		{
			...
			for(CPythonCharacterManager::CharacterIterator i = rkChrMgr.CharacterInstanceBegin(); i!=rkChrMgr.CharacterInstanceEnd();++i)
			{
				...
				if (m_GraphicThingInstance.TestActorCollision(pkInstEach->GetGraphicThingInstanceRef()) && !pkInstEach->IsShaman())
				{
					BlockMovement();
					return true;
				}
	...
	for(CPythonCharacterManager::CharacterIterator i = rkChrMgr.CharacterInstanceBegin(); i!=rkChrMgr.CharacterInstanceEnd();++i)
	{
		...
		if (!rkActorEach.GetRace() == 10 || !rkActorEach.GetRace() == 11)
			continue;

		if (rkActorSelf.TestActorCollision(rkActorEach) && !pkInstEach->IsShaman())
		{
			uCollisionCount++;
