...
UINT CInstanceBase::GetAlignmentGrade()
{
	...
	if (m_sAlignment >= 500000)
		return 0;
	else if (m_sAlignment >= 450000)
		return 1;
	else if (m_sAlignment >= 350000)
		return 2;
...
bool CInstanceBase::Create(const SCreateData& c_rkCreateData)
{
	...
	if (IsPC())
	{
		...
		SetEmblemat(c_rkCreateData.m_dwEmblemat);
#ifdef _DEBUG
		TraceError("Emblemat id from game: %d", c_rkCreateData.m_dwEmblemat);
#endif
#endif
	}
	...
	m_title_active = c_rkCreateData.m_title_active;
#endif
...
void CInstanceBase::__Create_SetName(const SCreateData& c_rkCreateData)
{
	...
	if (c_rkCreateData.m_stName.find("(") != std::string::npos)
	{
	...
	if (IsEnemy() && CPythonNonPlayer::Instance().IsAggressive(GetRace()))
	{
...
void wait(int s)
{
	unsigned long int t = GetTickCount() + (1000L * s);
...
bool CInstanceBase::SetRace(DWORD eRace)
{
	...
	if (m_dwRace == 30009)
	{
		m_GraphicThingInstance.SetScale(0.65, 0.65, 0.65, true);
	}
...
void CInstanceBase::Update()
{
	...
	{
		if (CPythonSystem::instance().GetShowAutoBuffMode() == 1)
		{
...
#ifdef ENABLE_NEW_PET_SYSTEM
	if ((GetRace() >= 34001 && GetRace() <= 34099) || (GetRace() >= 34901 && GetRace() <= 34999) 
		|| GetRace() == 34084 || GetRace() == 34085 || (GetRace() >= 34900 && GetRace() <= 34917))
	{
		if (CPythonSystem::instance().GetShowPetMode() == 1)
		{
			m_GraphicThingInstance.SetDeactiveAllAttachingEffect();
			__SetAffect(AFFECT_INVISIBILITY, true);
			m_kAffectFlagContainer.Set(AFFECT_INVISIBILITY, true);
	...
	if ((GetRace() >= 20900 && GetRace() <= 20913) || (GetRace() == 20204))
	{
		m_GraphicThingInstance.SetScale(0.5, 0.5, 0.5, true);
		m_GraphicThingInstance.SetDeactiveAllAttachingEffect();
		if (CPythonSystem::instance().GetShowMountMode() == 1)
		{
			m_GraphicThingInstance.SetDeactiveAllAttachingEffect();
			__SetAffect(AFFECT_INVISIBILITY, true);
			m_kAffectFlagContainer.Set(AFFECT_INVISIBILITY, true);
	...
	if (GetRace() == 30000 || GetRace() == 30001)
	{
		if (!__IsExistMainInstance())
			return;
		...
		if (fDistance >= CPythonSystem::instance().GetShowOfflineShopRange())
		{
			m_GraphicThingInstance.SetDeactiveAllAttachingEffect();
			__SetAffect(AFFECT_INVISIBILITY, true);
			m_kAffectFlagContainer.Set(AFFECT_INVISIBILITY, true);
...
#endif
#ifdef ENABLE_COSTUME_EMBLEMAT
	DWORD eEmblemat = GetPart(CRaceData::PART_EMBLEMAT);
#endif
...
#endif
#ifdef ENABLE_COSTUME_EMBLEMAT
	SetEmblemat(eEmblemat);
#ifdef _DEBUG
	TraceError("Emblemat id from game: %d", eEmblemat);
#endif
#endif
	SetRotation(fRot);
...
#ifdef ENABLE_COSTUME_EMBLEMAT
bool CInstanceBase::SetEmblemat(DWORD eEmblemat)
{
	if (!IsPC() || IsPoly() || __IsShapeAnimalWear())
		return false;

	m_GraphicThingInstance.AttachEmblemat(eEmblemat, 0, CRaceData::PART_EMBLEMAT);
#ifdef _DEBUG
	TraceError("EeEmblemat: %d", eEmblemat);
#endif
	if (!eEmblemat)
	{
#ifdef _DEBUG
		TraceError(" eEmblemat nie istnieje kekw");
#endif
		if (m_emblematEffect)
		{
#ifdef _DEBUG
			TraceError("__DetachEffect emblemat: %d", eEmblemat);
#endif
			__DetachEffect(m_emblematEffect);
			m_emblematEffect = 0;
		}
		m_awPart[CRaceData::PART_EMBLEMAT] = 0;
		return true;
	}
#ifdef _DEBUG
	TraceError("Emblemat effect: %d", m_emblematEffect);
#endif
	CItemData* pItemData;
	if (!CItemManager::Instance().GetItemDataPointer(eEmblemat, &pItemData))
	{
		if (m_emblematEffect)
		{
			__DetachEffect(m_emblematEffect);
...
#ifdef _DEBUG
		TraceError("CItemManager::Instance().GetItemDataPointer instancebase ok");
#endif
		m_awPart[CRaceData::PART_EMBLEMAT] = 0;
		return true;
	}

	D3DXVECTOR3 scalePos(0.0, 0.0, 0.0);
	...
	D3DXVECTOR3 v3Dir = (pCamera->GetEye() - v3Pos) * 1 / 10;
	v3Pos = pCamera->GetEye() - v3Dir;

	v3Pos = D3DXVECTOR3(0, 0, 0);
	if (m_kHorse.IsMounting()) {
		v3Pos.z += float(m_GraphicThingInstance.GetHeight() + 20.0f);
	}
	else {
		v3Pos.z += float(m_GraphicThingInstance.GetHeight() - 30.0f);
	}

#ifdef _DEBUG
	TraceError("Emblemat scale ok");
#endif
	m_emblematEffect = m_GraphicThingInstance.AttachEffectByID(0, NULL, pItemData->GetEmblematEffectID(), &v3Pos);
	m_awPart[CRaceData::PART_EMBLEMAT] = eEmblemat;
#ifdef _DEBUG
	TraceError("Emblemat attach ok");
#endif
	return true;
}

void CInstanceBase::ChangeEmblemat(DWORD eEmblemat)
{
	if (eEmblemat == m_GraphicThingInstance.GetPartItemID(CRaceData::PART_EMBLEMAT))
		return;

	if (SetEmblemat(eEmblemat))
		RefreshState(CRaceMotionData::NAME_WAIT, true);
}

void CInstanceBase::__ClearEmblematRefineEffect()
{
	if (!m_emblematEffect)
		return;

	__EffectContainer_DetachEffect(m_emblematEffect);
	m_emblematEffect = 0;
}
#endif
#ifdef ENABLE_QUIVER_SYSTEM
