...
const std::vector<int> miniBossy = { 9711, 1091, 2093, 29753, 29755, 16100, 16101, 9695, 9683, 9684, 16160 };
...
void CPythonCharacterManager::SetDogMode(bool bStatus)
{
	...
	for (auto& it : m_kAliveInstMap)
	{
		...
		if (pInstance && pMainInstance && pMainInstance != pInstance && (pInstance->IsEnemy() || pInstance->IsStone()))
		{
			if ((bStatus))
			{
				if (pInstance->GetOriginalRace() == pInstance->GetRace() && pInstance->GetRace() != 101 
					&& pMobTable->bRank < 4
					&& std::find(miniBossy.begin(), miniBossy.end(), pInstance->GetRace()) == miniBossy.end())
				{
