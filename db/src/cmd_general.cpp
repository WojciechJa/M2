...
ACMD(do_event_manager)
{
    ...
    split_argument(argument, vecArgs);
    if (vecArgs.size() < 2) { 
        ch->ChatPacket(CHAT_TYPE_INFO, "Usage: event_manager <info|add|remove|update>");
        return; 
    }
    ...
    else if (vecArgs[1] == "add")
    {
        if (!ch->IsGM())
            return;

        if (vecArgs.size() < 9) { // Zmienione na 9 bo potrzebujemy 9 argumentów (7 + 2 flagi)
            ch->ChatPacket(CHAT_TYPE_INFO, "Usage: event_manager add <eventIndex> <startDate> <startTime> <endDate> <endTime> <empireFlag> <channelFlag>");
            ch->ChatPacket(CHAT_TYPE_INFO, "Event Types:");
            ch->ChatPacket(CHAT_TYPE_INFO, "1: BONUS_EVENT");
            ch->ChatPacket(CHAT_TYPE_INFO, "2: DOUBLE_BOSS_LOOT_EVENT");
            ch->ChatPacket(CHAT_TYPE_INFO, "3: DOUBLE_METIN_LOOT_EVENT");
            ch->ChatPacket(CHAT_TYPE_INFO, "4: DOUBLE_MISSION_BOOK_EVENT");
            ch->ChatPacket(CHAT_TYPE_INFO, "5: DUNGEON_COOLDOWN_EVENT");
            ch->ChatPacket(CHAT_TYPE_INFO, "6: DUNGEON_CHEST_LOOT_EVENT");
            ch->ChatPacket(CHAT_TYPE_INFO, "7: EMPIRE_WAR_EVENT");
            ch->ChatPacket(CHAT_TYPE_INFO, "8: MOONLIGHT_EVENT");
            ch->ChatPacket(CHAT_TYPE_INFO, "9: TOURNAMENT_EVENT");
            ch->ChatPacket(CHAT_TYPE_INFO, "10: WHELL_OF_FORTUNE_EVENT");
            ch->ChatPacket(CHAT_TYPE_INFO, "11: HALLOWEEN_EVENT");
            ch->ChatPacket(CHAT_TYPE_INFO, "12: NPC_SEARCH_EVENT");
            ch->ChatPacket(CHAT_TYPE_INFO, "13: EXP_EVENT");
            ch->ChatPacket(CHAT_TYPE_INFO, "14: ITEM_DROP_EVENT");
            ch->ChatPacket(CHAT_TYPE_INFO, "15: YANG_DROP_EVENT");
            ch->ChatPacket(CHAT_TYPE_INFO, "Empire Flag: 0-All, 1-Red, 2-Yellow, 3-Blue");
            ch->ChatPacket(CHAT_TYPE_INFO, "Channel Flag: 0-All, 1-99 specific channel");
            ch->ChatPacket(CHAT_TYPE_INFO, "Optional: [value0] [value1] [value2] [value3]");
            return;
        }
        ...
        // Parsowanie event index
        BYTE eventIndex;
        str_to_number(eventIndex, vecArgs[2].c_str());
        
        if (eventIndex < BONUS_EVENT || eventIndex > YANG_DROP_EVENT) {
            ch->ChatPacket(CHAT_TYPE_INFO, "Invalid event index (1-15)!");
            return;
        }
        ...
        // Debug info
        ch->ChatPacket(CHAT_TYPE_INFO, "Debug: Liczba argumentów: %zu", vecArgs.size());

        // Laczymy date i czas w jeden string
        std::string startTimeStr = vecArgs[3] + " " + vecArgs[4];
        std::string endTimeStr = vecArgs[5] + " " + vecArgs[6];

        ch->ChatPacket(CHAT_TYPE_INFO, "Debug: Combined start time: %s", startTimeStr.c_str());
        ch->ChatPacket(CHAT_TYPE_INFO, "Debug: Combined end time: %s", endTimeStr.c_str());

        // Parsowanie czasów
        time_t currentTime = time(0);
        struct tm startTm = *localtime(&currentTime);
        struct tm endTm = *localtime(&currentTime);

        // Parsowanie czasu rozpoczecia
        int year, mon, day, hour, min, sec;
        int result = sscanf(startTimeStr.c_str(), "%d-%d-%d %d:%d:%d",
            &year, &mon, &day, &hour, &min, &sec);

        if (result != 6)
        {
            ch->ChatPacket(CHAT_TYPE_INFO, "Invalid start time format! Use: YYYY-MM-DD HH:MM:SS");
            ch->ChatPacket(CHAT_TYPE_INFO, "Debug: sscanf result: %d (should be 6)", result);
            return;
        }

        startTm.tm_year = year - 1900;
        startTm.tm_mon = mon - 1;
        startTm.tm_mday = day;
        startTm.tm_hour = hour;
        startTm.tm_min = min;
        startTm.tm_sec = sec;

        // Parsowanie czasu zakonczenia
        result = sscanf(endTimeStr.c_str(), "%d-%d-%d %d:%d:%d",
            &year, &mon, &day, &hour, &min, &sec);

        if (result != 6)
        {
            ch->ChatPacket(CHAT_TYPE_INFO, "Invalid end time format! Use: YYYY-MM-DD HH:MM:SS");
            ch->ChatPacket(CHAT_TYPE_INFO, "Debug: sscanf result: %d (should be 6)", result);
            return;
        }
        ...
        eventData.startTime = mktime(&startTm);
        eventData.endTime = IsDontHaveEndTimeEvent(eventIndex) ? 0 : mktime(&endTm);

        if (!IsDontHaveEndTimeEvent(eventIndex) && eventData.endTime <= eventData.startTime) {
            ch->ChatPacket(CHAT_TYPE_INFO, "End time must be after start time!");
            return;
        }

        SetTimeToString(&eventData);

        // Parsowanie flag
        BYTE empireFlag, channelFlag;

        // Debug dla flag
        ch->ChatPacket(CHAT_TYPE_INFO, "Debug: Empire flag argument: %s", vecArgs[7].c_str());
        ch->ChatPacket(CHAT_TYPE_INFO, "Debug: Channel flag argument: %s", vecArgs[8].c_str());

        str_to_number(empireFlag, vecArgs[7].c_str());
        str_to_number(channelFlag, vecArgs[8].c_str());

        if (empireFlag > 3) {
            ch->ChatPacket(CHAT_TYPE_INFO, "Invalid empire flag (0-3)!");
            return;
        }

        if (channelFlag > 99) {
            ch->ChatPacket(CHAT_TYPE_INFO, "Invalid channel flag (0-99)!");
            return;
        }

        eventData.empireFlag = empireFlag;
        eventData.channelFlag = channelFlag;

        // Opcjonalne wartosci
        if (vecArgs.size() > 9) str_to_number(eventData.value[0], vecArgs[9].c_str());
        if (vecArgs.size() > 10) str_to_number(eventData.value[1], vecArgs[10].c_str());
        if (vecArgs.size() > 11) str_to_number(eventData.value[2], vecArgs[11].c_str());
        if (vecArgs.size() > 12) str_to_number(eventData.value[3], vecArgs[12].c_str());

        // Status bazowy
        eventData.eventStatus = false;
        eventData.eventTypeOnlyStart = IsDontHaveEndTimeEvent(eventData.eventIndex);

        // Wysylamy do DB
        const BYTE subHeader = EVENT_MANAGER_LOAD;
        db_clientdesc->DBPacketHeader(HEADER_GD_EVENT_MANAGER, 0, sizeof(BYTE) + sizeof(TEventManagerData));
        db_clientdesc->Packet(&subHeader, sizeof(BYTE));
        db_clientdesc->Packet(&eventData, sizeof(TEventManagerData));

        ch->ChatPacket(CHAT_TYPE_INFO, "Event added successfully!");
    }
    else if (vecArgs[1] == "remove")
    {
        if (!ch->IsGM())
        ...
        if (vecArgs.size() < 3) { 
            ch->ChatPacket(CHAT_TYPE_INFO, "Usage: event_manager remove <eventID>");
            return; 
        }

        WORD eventID;
        str_to_number(eventID, vecArgs[2].c_str());

        // Wysylamy zadanie usuniecia
        ...
        db_clientdesc->Packet(&subHeader, sizeof(BYTE));
        db_clientdesc->Packet(&eventID, sizeof(WORD));

        ch->ChatPacket(CHAT_TYPE_INFO, "Event removal request sent!");
    }
    else if (vecArgs[1] == "update")
    {
        if (!ch->IsGM())
        ...
        ch->ChatPacket(CHAT_TYPE_INFO, "Event update request sent!");
    }
}
#endif
///////TESTTESTEST/////
