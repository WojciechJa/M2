...
void CClientManager::UpdatePetName(TPacketUpdateHorseName* data, CPeer* peer)
{
	char szQuery[512];

	snprintf(szQuery, sizeof(szQuery), "REPLACE INTO pet_name VALUES(%u, '%s')", data->dwPlayerID, data->szHorseName);

	std::unique_ptr<SQLMsg> pmsg_insert(CDBManager::instance().DirectQuery(szQuery));

	ForwardPacket(HEADER_DG_UPDATE_PET_NAME, data, sizeof(TPacketUpdateHorseName), 0, peer);
}

void CClientManager::AckPetName(DWORD dwPID, CPeer* peer)
{
	char szQuery[512];

	snprintf(szQuery, sizeof(szQuery), "SELECT name FROM pet_name WHERE id = %u", dwPID);
	...
	peer->EncodeHeader(HEADER_DG_ACK_PET_NAME, 0, sizeof(TPacketUpdateHorseName));
	peer->Encode(&packet, sizeof(TPacketUpdateHorseName));
}
#endif
