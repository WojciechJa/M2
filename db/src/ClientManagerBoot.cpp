...
bool CClientManager::InitializeTables()
{
	...
	if (!InitializeGloveAttrTable())
	{
		sys_err("InitializeGloveAttrTable FAILED");
		return false;
	}
#endif
	...
	if (!InitializeBattlePassQuest())
	{
		sys_err("InitializeBattlePassQuest FAILED");
		return false;
	}
	
	if (!InitializeBattlePassReward())
	{
		sys_err("InitializeBattlePassReward FAILED");
		return false;
	}
	
	if (!InitializeBattlePassInformation())
	{
		sys_err("InitializeBattlePassInformation FAILED");
		return false;
	}
#endif
...
bool CClientManager::InitializeRefineTable()
{
	...
	snprintf(query, sizeof(query),
			"SELECT id, cost, cost2, cost3, prob, vnum0, count0, vnum1, count1, vnum2, count2,  vnum3, count3, vnum4, count4, vnum5, count5, vnum6, count6, vnum7, count7, vnum8, count8, vnum9, count9 FROM refine_proto%s",
			GetTablePostfix());
	...
	while ((data = mysql_fetch_row(pRes->pSQLResult)))
	{
		...
		str_to_number(prt->cost2, data[col++]);
		str_to_number(prt->cost3, data[col++]);
		...
		sys_log(0, "REFINE: id %ld cost %d cost2 %d cost3 %d prob %d mat1 %lu cnt1 %d", prt->id, prt->cost, prt->cost2, prt->cost3, prt->prob, prt->materials[0].vnum, prt->materials[0].count);
...
bool CClientManager::InitializeBeltAttrTable()
{
	char query[4096];
	snprintf(query, sizeof(query),
			"SELECT apply, apply+0, prob, lv1, lv2, lv3, lv4, lv5, belt "
			" FROM item_belt_attr%s ORDER BY apply",
			GetTablePostfix());
	...
	while ((data = mysql_fetch_row(pRes->pSQLResult)))
	{
		...
		sys_log(0, "BELT_ATTR: %-20s %4lu { %3d %3d %3d %3d %3d } { %d"
				" }",
				t.szApply,
				t.dwProb,
				t.lValues[0],
		...
		m_vec_beltAttrTable.emplace_back(t);
	}
...
#ifdef ENABLE_GLOVE_ATTRIBUTES
bool CClientManager::InitializeGloveAttrTable()
{
	char query[4096];
	snprintf(query, sizeof(query),
			"SELECT apply, apply+0, prob, lv1, lv2, lv3, lv4, lv5, glove "
			" FROM item_glove_attr%s ORDER BY apply",
			GetTablePostfix());
	...
	if (!pRes->uiNumRows)
	{
		sys_err("no result from glove_attr");
		return false;
	}

	if (!m_vec_gloveAttrTable.empty())
	{
		sys_log(0, "RELOAD: glove_attr");
		m_vec_gloveAttrTable.clear();
	}

	m_vec_gloveAttrTable.reserve(pRes->uiNumRows);
	...
	while ((data = mysql_fetch_row(pRes->pSQLResult)))
	{
		...
		sys_log(0, "GLOVE_ATTR: %-20s %4lu { %3d %3d %3d %3d %3d } { %d"
				" }",
				t.szApply,
				t.dwProb,
				t.lValues[0],
		...
		m_vec_gloveAttrTable.emplace_back(t);
	}
	...
	for (itertype(m_vec_itemTable) it = m_vec_itemTable.begin(); it != m_vec_itemTable.end(); it++)
	{
		if (g_stLocaleNameColumn != "name")
		{
			...
			snprintf(query, sizeof(query),
				...
				"refined_vnum, refine_set, magic_pct, socket_pct, addon_type, special_inv_type, can_use_specialinv, "
#else
...
#ifdef ENABLE_SPLIT_INVENTORY_SYSTEM
				t.dwRefinedVnum, t.wRefineSet, t.bAlterToMagicItemPct, t.bGainSocketPct, t.sAddonType, t.SpecialInvType, t.CanUseInSpecialInv, //20
#else
...
#else
				t.dwRefinedVnum, t.wRefineSet, t.bAlterToMagicItemPct, t.bGainSocketPct, t.sAddonType, t.SpecialInvType, t.CanUseInSpecialInv,
#endif
...
#ifdef ENABLE_PUNKTY_OSIAGNIEC
		VERIFY_IFIELD(MProto::pkt_osiag,		mob_table->sPktOsiag);
#endif

		sys_log(0, "MOB #%-5d %-24s %-24s level: %-3u rank: %u empire: %d max_hp: %lld",
			mob_table->dwVnum,
			mob_table->szName,
...
#ifdef ENABLE_SPLIT_INVENTORY_SYSTEM
	socket_pct, addon_type, special_inv_type, can_use_specialinv, limittype0, limitvalue0, limittype1, limitvalue1,
#else
...
#ifdef ENABLE_SPLIT_INVENTORY_SYSTEM
		VERIFY_IFIELD(IProto::special_inv_type,		item_table->SpecialInvType);
		VERIFY_IFIELD(IProto::can_use_specialinv,	item_table->CanUseInSpecialInv);
#endif
#if !defined(ENABLE_AUTODETECT_VNUMRANGE)
...
#ifdef ENABLE_LRN_BATTLE_PASS
bool CClientManager::InitializeBattlePassQuest()
{
	std::string query = "SHOW TABLES LIKE 'quest_battle_pass" + std::string(GetTablePostfix()) + "'";
	
	auto pkMsg(CDBManager::instance().DirectQuery(query.c_str()));
	SQLResult * pRes = pkMsg->Get();
	
	return pRes && pRes->uiNumRows > 0;
}

bool CClientManager::InitializeBattlePassReward()
{
	std::string query = "SHOW TABLES LIKE 'reward_battle_pass" + std::string(GetTablePostfix()) + "'";
...
bool CClientManager::InitializeBattlePassInformation()
{
	std::string query = "SHOW TABLES LIKE 'player_battle_pass" + std::string(GetTablePostfix()) + "'";
