...
class CClientManager : public CNetBase, public singleton<CClientManager>
{
	...
	typedef boost::unordered_set<CPrivateShopItemCache*, std::hash<CPrivateShopItemCache*> > TPrivateShopItemCacheSet;
	...
	typedef boost::unordered_set<CPrivateShopSaleCache*, std::hash<CPrivateShopSaleCache*> > TPrivateShopSaleCacheSet;
...
#ifdef ENABLE_LRN_BATTLE_PASS
	bool		InitializeBattlePassQuest();
	bool		InitializeBattlePassReward();
	bool		InitializeBattlePassInformation();
#endif
...
#ifdef ENABLE_LRN_BATTLE_PASS
	void		RESULT_BATTLE_PASS_QUEST_LOAD(CPeer * pkPeer, MYSQL_RES * pRes, DWORD dwHandle, DWORD dwPID);
	void		RESULT_BATTLE_PASS_REWARD_LOAD(CPeer * pkPeer, MYSQL_RES * pRes, DWORD dwHandle, DWORD dwPID);
	void		RESULT_BATTLE_PASS_INFO_LOAD(CPeer * pkPeer, MYSQL_RES * pRes, DWORD dwHandle, DWORD dwPID);
	
	template<typename RowMapper>
	void		EncodeBattlePassData(CPeer * pkPeer, MYSQL_RES * pRes, DWORD dwHandle, DWORD dwPID, int dataSize, BYTE header, RowMapper rowMapper);
#endif
...
#ifdef ENABLE_LRN_BATTLE_PASS
	void		QUERY_BATTLE_PASS_QUEST_SAVE(CPeer * pkPeer, TBattlePassQuestTable *, DWORD dwLen);
	void		QUERY_BATTLE_PASS_REWARD_SAVE(CPeer * pkPeer, TBattlePassRewardTable *, DWORD dwLen);
	void		QUERY_BATTLE_PASS_INFO_SAVE(CPeer * pkPeer, TBattlePassInformationTable *, DWORD dwLen);
#endif
...
#ifdef ENABLE_LRN_LOCATION_SYSTEM
private:
	typedef std::unordered_map<int, CLocationSystemCache*> TLocationSystemCacheMap;
	TLocationSystemCacheMap m_mapLocationSystemCache;
	
public:
	CLocationSystemCache* GetLocationSystemCache(DWORD pid);
	
	void		CreateLocationSystemCache(DWORD pid);
	void		UpdateLocationSystemCache(DWORD pid);
	void		InsertLocationSystemCache(TLocationSystemData &data, bool bSkip = false);
	
	void		GetLocationSystemData(CPeer* peer, DWORD dwHandle, const char* data);
	void		UpdateLocationSystemData();
	
	void		ManagerLocationSystemData(CPeer* peer, DWORD dwHandle, const char* data);
#endif

#ifdef ENABLE_LRN_BATTLE_PASS
private:
	uint32_t	BattlePassResetTime = 0;
	
public:
	void		SetBattlePassReset();
	void		UsageBattlePassReset();
#endif
...
#ifdef __ITEM_SHOP__
	void ItemShopUpdatePremium(TItemShopSavePremiumPacket* data);
#endif
