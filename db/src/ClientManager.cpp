...
void CClientManager::MainLoop()
{
	...
	for (itertype(m_mapLocationSystemCache) itLocationSystem = m_mapLocationSystemCache.begin(); itLocationSystem != m_mapLocationSystemCache.end(); ++itLocationSystem)
	{
		CLocationSystemCache* pCache = itLocationSystem->second;
		pCache->Flush();
		
		delete pCache;
	}
	
	m_mapLocationSystemCache.clear();
#endif
...
void CClientManager::QUERY_BOOT(CPeer* peer, TPacketGDBoot * p)
{
	...
	DWORD dwPacketSize =
		...
		sizeof(WORD) + sizeof(WORD) + sizeof(TItemAttrTable) * m_vec_beltAttrTable.size() +
#endif
#ifdef ENABLE_GLOVE_ATTRIBUTES
		sizeof(WORD) + sizeof(WORD) + sizeof(TItemAttrTable) * m_vec_gloveAttrTable.size() +
#endif
...
#ifdef ENABLE_BELT_ATTRIBUTES
	peer->EncodeWORD(sizeof(TItemAttrTable));
	peer->EncodeWORD(m_vec_beltAttrTable.size());
	peer->Encode(&m_vec_beltAttrTable[0], sizeof(TItemAttrTable) * m_vec_beltAttrTable.size());
#endif
#ifdef ENABLE_GLOVE_ATTRIBUTES
	peer->EncodeWORD(sizeof(TItemAttrTable));
	peer->EncodeWORD(m_vec_gloveAttrTable.size());
	peer->Encode(&m_vec_gloveAttrTable[0], sizeof(TItemAttrTable) * m_vec_gloveAttrTable.size());
#endif
...
#ifdef ENABLE_BELT_ATTRIBUTES
				sizeof(WORD) + sizeof(TItemAttrTable)*m_vec_beltAttrTable.size() +
#endif
#ifdef ENABLE_GLOVE_ATTRIBUTES
				sizeof(WORD) + sizeof(TItemAttrTable)*m_vec_gloveAttrTable.size());
#endif
...
#ifdef ENABLE_BELT_ATTRIBUTES
		tmp->EncodeWORD(m_vec_beltAttrTable.size());
		tmp->Encode(&m_vec_beltAttrTable[0], sizeof(TItemAttrTable) * m_vec_beltAttrTable.size());
#endif
#ifdef ENABLE_GLOVE_ATTRIBUTES
		tmp->EncodeWORD(m_vec_gloveAttrTable.size());
		tmp->Encode(&m_vec_gloveAttrTable[0], sizeof(TItemAttrTable) * m_vec_gloveAttrTable.size());
#endif
	}
}
...
void CClientManager::ProcessPackets(CPeer * peer)
{
	...
	while (peer->PeekPacket(i, header, dwHandle, dwLength, &data))
	{
		...
		switch (header)
		{
			...
			case HEADER_GD_UPDATE_PET_NAME:
				UpdatePetName((TPacketUpdateHorseName*)data, peer);
				break;

			case HEADER_GD_REQ_PET_NAME:
				AckPetName(*(DWORD*)data, peer);
				break;
#endif
...
#endif
#ifdef ENABLE_LRN_LOCATION_SYSTEM
			case HEADER_GD_LOCATION_SYSTEM_GET_DATA:
				GetLocationSystemData(peer, dwHandle, data);
				break;
				
			case HEADER_GD_LOCATION_SYSTEM_MANAGER_DATA:
				ManagerLocationSystemData(peer, dwHandle, data);
				break;
#endif
#ifdef __ITEM_SHOP__
			case HEADER_GD_REFRESH_ITEMSHOP:
				sys_log(0, "HEADER_GD_REFRESH_ITEMSHOP (handle: %d length: %d)", dwHandle, dwLength);
				ForwardPacket(HEADER_DG_REFRESH_ITEMSHOP, (TItemShopRefreshPacket*)data, sizeof(TItemShopRefreshPacket));
				break;

			case HEADER_GD_ITEMSHOP_SAVE_PREMIUM:
				sys_log(0, "HEADER_GD_ITEMSHOP_SAVE_PREMIUM (handle: %d length: %d)", dwHandle, dwLength);
				ItemShopUpdatePremium((TItemShopSavePremiumPacket*)data);
				break;
#endif
#ifdef WEEKLY_RANK_BYLUZER
			case HEADER_GD_WEEKLY_REWARDS:
				WeeklyRewards(peer, (TWeeklyRewardsGD *) data);
				break;
#endif
#ifdef ENABLE_LRN_BATTLE_PASS
			case HEADER_GD_BATTLE_PASS_QUEST_SAVE:
				QUERY_BATTLE_PASS_QUEST_SAVE(peer, (TBattlePassQuestTable *) data, dwLength);
				break;
			
			case HEADER_GD_BATTLE_PASS_REWARD_SAVE:
				QUERY_BATTLE_PASS_REWARD_SAVE(peer, (TBattlePassRewardTable *) data, dwLength);
				break;
			
			case HEADER_GD_BATTLE_PASS_INFO_SAVE:
				QUERY_BATTLE_PASS_INFO_SAVE(peer, (TBattlePassInformationTable *) data, dwLength);
				break;
#endif
#ifdef ENABLE_EVENT_MANAGER
...
#ifdef ENABLE_LRN_BATTLE_PASS
		case QID_BATTLE_PASS_QUEST_SAVE:
		case QID_BATTLE_PASS_REWARD_SAVE:
		case QID_BATTLE_PASS_INFO_SAVE:
#endif
#ifdef __PREMIUM_PRIVATE_SHOP__
...
#ifdef ENABLE_LRN_BATTLE_PASS
			if (!(thecore_heart->pulse % (thecore_heart->passes_per_sec * 1)))
				UsageBattlePassReset();
#endif
...
#ifdef ENABLE_LRN_BATTLE_PASS
void CClientManager::SetBattlePassReset()
{
	auto now = std::chrono::system_clock::now();
	auto now_time = std::chrono::system_clock::to_time_t(now);
	std::tm* local_tm = std::localtime(&now_time);
	
	std::tm end_of_month_tm = *local_tm;
	end_of_month_tm.tm_mon++;
	end_of_month_tm.tm_mday = 1;
	end_of_month_tm.tm_hour = 0;
	end_of_month_tm.tm_min = 0;
	end_of_month_tm.tm_sec = 0;
	std::mktime(&end_of_month_tm);
	
	auto end_of_month = std::chrono::system_clock::from_time_t(std::mktime(&end_of_month_tm));
	auto diff_seconds = std::chrono::duration_cast<std::chrono::seconds>(end_of_month - now);
	auto current_seconds = std::chrono::duration_cast<std::chrono::seconds>(now.time_since_epoch());
	diff_seconds += current_seconds;
	
	BattlePassResetTime = static_cast<uint32_t>(diff_seconds.count());
}

void CClientManager::UsageBattlePassReset()
{
	auto currentTime = std::chrono::system_clock::now();
	auto currentTimeSeconds = std::chrono::time_point_cast<std::chrono::seconds>(currentTime);
	std::time_t currentTimeInSeconds = std::chrono::system_clock::to_time_t(currentTimeSeconds);
	
	if (currentTimeInSeconds == BattlePassResetTime)
	{
		ForwardPacket(HEADER_DG_BATTLE_PASS_CLEAR, NULL, 0);
		
		auto postfix = GetTablePostfix();
		std::string postfixStr = postfix;
		
		auto execute_query = [&](const std::string& query) { CDBManager::instance().DirectQuery(query.c_str()); };
		
		execute_query("TRUNCATE TABLE quest_battle_pass" + postfixStr);
		execute_query("TRUNCATE TABLE reward_battle_pass" + postfixStr);
		execute_query("UPDATE player_battle_pass" + postfixStr + " SET level = 1, exp = 0, premium = 0");
	}
}
#endif
...
#ifdef __ITEM_SHOP__
void CClientManager::ItemShopUpdatePremium(TItemShopSavePremiumPacket* data)
{
	CLoginData* pkLD = GetLoginDataByAID(data->account_id);

	if (!pkLD)
	{
		sys_err("cannot find login data for account %d", data->account_id);
		return;
	}

	int* tPremium = pkLD->GetPremiumPtr();

	std::string stPremiumRow;
	switch (data->premium)
	{
		case PREMIUM_EXP:
			stPremiumRow = "vip_pd_expire";
			break;
		case PREMIUM_ITEM:
			stPremiumRow = "vip_glove_expire";
			break;
		case PREMIUM_SAFEBOX:
			stPremiumRow = "safebox_expire";
			break;
		case PREMIUM_AUTOLOOT:
			stPremiumRow = "autoloot_expire";
			break;
		case PREMIUM_FISH_MIND:
			stPremiumRow = "fish_mind_expire";
			break;
		case PREMIUM_MARRIAGE_FAST:
			stPremiumRow = "marriage_fast_expire";
			break;
		case PREMIUM_GOLD:
			stPremiumRow = "money_drop_rate_expire";
			break;
		default:
			sys_err("Premium type %d is undefined", data->premium);
			return;
	}

	tPremium[data->premium] = data->time;

	char szQuery[256];
	snprintf(szQuery, sizeof(szQuery), "UPDATE `account` SET `%s` = FROM_UNIXTIME(%d) WHERE id = %d",
		stPremiumRow.c_str(), data->time, data->account_id);
...
#ifdef WEEKLY_RANK_BYLUZER
void CClientManager::WeeklyRewards(CPeer * pkPeer, TWeeklyRewardsGD * p)
{
	if (p->item_vnum <= 0)
		return;
	GiveItemRewardToPlayer(p->account_id, p->item_vnum);
}
#endif
...
int CClientManager::GetEmptyMallPosition(BYTE itemSize, DWORD accountID)
{
	...
	{
		str_to_number(itemPos, row[0]);
		str_to_number(itemVnum, row[1]);

		TItemTable * itemTab = m_map_itemTableByVnum.find(itemVnum)->second;
		if (!itemTab)
			continue;
		for (int i = 0; i < itemTab->bSize; i++)
			slots[itemPos+5*i] = true;
	}

	for (int i = 0; i < 50-5*itemSize; i++)//itemsize == 1 jezeli bedzie wolna pozycja 40-45 nie zwroci poprawnej wartosci :)
	{
		bool isEmpty = true;
		for (int j = 0; j < itemSize; j++)//z�a zmienna
		{
			if (slots[i+5*j])
				isEmpty = false;
		}
...
void CClientManager::GiveItemRewardToPlayer(DWORD accountID, DWORD itemVnum)
{
	TItemTable * rewardItemTable = m_map_itemTableByVnum.find(itemVnum)->second;

	if (!rewardItemTable)
	{
        sys_err("cannot find item by vnum=%d", itemVnum);
		return;
	}

	int emptyPos = GetEmptyMallPosition(rewardItemTable->bSize, accountID);

	if (emptyPos == -1)
	{
        sys_err("cannot find empty position for item %d", itemVnum);
		return;
	}

	DWORD newItemID = GainItemID();

	char queryItem[QUERY_MAX_LEN];
	snprintf(queryItem, sizeof(queryItem), "INSERT INTO item (id, owner_id, window, pos, vnum, count, socket0, socket1, socket2, socket3, socket4, socket5) VALUES(%d, %d, 'MALL', %d, %d, 1, 0, 0, 0, 0, 0, 0)", newItemID, accountID, emptyPos, itemVnum);
	CDBManager::instance().DirectQuery(queryItem);
}
#ifdef ENABLE_LRN_BATTLE_PASS
void CClientManager::QUERY_BATTLE_PASS_QUEST_SAVE(CPeer * pkPeer, TBattlePassQuestTable * pTable, DWORD dwLen)
{
	if (0 != (dwLen % sizeof(TBattlePassQuestTable)))
	{
		sys_err("invalid packet size %d, sizeof(TBattlePassQuestTable) == %d", dwLen, sizeof(TBattlePassQuestTable));
		return;
	}

	int iSize = dwLen / sizeof(TBattlePassQuestTable);
	char szQuery[1024];
	
	for (int i = 0; i < iSize; ++i, ++pTable)
	{
		snprintf(szQuery, sizeof(szQuery),
				"INSERT INTO quest_battle_pass%s (pid, qidx, count, status) VALUES (%d, %d, %d, %d) ON DUPLICATE KEY UPDATE count = %d, status = %d",
				GetTablePostfix(), pTable->pid, pTable->index, pTable->count, pTable->status, pTable->count, pTable->status);

		CDBManager::instance().ReturnQuery(szQuery, QID_BATTLE_PASS_QUEST_SAVE, pkPeer->GetHandle(), NULL);
	}
}

void CClientManager::QUERY_BATTLE_PASS_REWARD_SAVE(CPeer * pkPeer, TBattlePassRewardTable * pTable, DWORD dwLen)
{
	if (0 != (dwLen % sizeof(TBattlePassRewardTable)))
	{
		sys_err("invalid packet size %d, sizeof(TBattlePassRewardTable) == %d", dwLen, sizeof(TBattlePassRewardTable));
		return;
	}

	int iSize = dwLen / sizeof(TBattlePassRewardTable);
	char szQuery[1024];
	
	for (int i = 0; i < iSize; ++i, ++pTable)
	{
		snprintf(szQuery, sizeof(szQuery),
				"INSERT INTO reward_battle_pass%s (pid, level, status_free, status_premium) VALUES (%d, %d, %d, %d) ON DUPLICATE KEY UPDATE status_free = %d, status_premium = %d",
				GetTablePostfix(), pTable->pid, pTable->level, pTable->status_free, pTable->status_premium, pTable->status_free, pTable->status_premium);

		CDBManager::instance().ReturnQuery(szQuery, QID_BATTLE_PASS_QUEST_SAVE, pkPeer->GetHandle(), NULL);
	}
}

void CClientManager::QUERY_BATTLE_PASS_INFO_SAVE(CPeer * pkPeer, TBattlePassInformationTable * pTable, DWORD dwLen)
{
	if (0 != (dwLen % sizeof(TBattlePassInformationTable)))
	{
		sys_err("invalid packet size %d, sizeof(TBattlePassInformationTable) == %d", dwLen, sizeof(TBattlePassInformationTable));
		return;
	}

	int iSize = dwLen / sizeof(TBattlePassInformationTable);
	char szQuery[1024];
	
	for (int i = 0; i < iSize; ++i, ++pTable)
	{
		snprintf(szQuery, sizeof(szQuery),
				"INSERT INTO player_battle_pass%s (pid, level, exp, premium) VALUES (%d, %d, %d, %d) ON DUPLICATE KEY UPDATE level = %d, exp = %d, premium = %d",
				GetTablePostfix(), pTable->pid, pTable->level, pTable->exp, pTable->premium, pTable->level, pTable->exp, pTable->premium);

		CDBManager::instance().ReturnQuery(szQuery, QID_BATTLE_PASS_INFO_SAVE, pkPeer->GetHandle(), NULL);
	}
}
#endif
