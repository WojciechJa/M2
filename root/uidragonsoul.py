...
class DragonSoulWindow(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		self.toolTip[1].SetTitle("Odnawianie czasu")
		self.toolTip[1].AppendSpace(3)
		self.toolTip[1].AppendHorizontalLine()
		self.toolTip[1].AppendSpace(3)
		self.toolTip[1].AppendTextLine("Koszt: |cFFc99f36200.000.000 {}".format(emoji.AppendEmoji("icon/emoji/money_icon.png")))
		...
		try:
			if self.wndDragonSoulRefine.IsShow():
				if uiPrivateShopBuilder.IsBuildingPrivateShop():
					chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.MOVE_ITEM_FAILURE_PRIVATE_SHOP)
					return
				if uiOfflineShop.IsEditingOfflineShop() or uiOfflineShopBuilder.IsBuildingOfflineShop():
	...
	def UseItemFastSlotTest(self, slot):
		...
		# else:
			# chat.AppendChat(chat.CHAT_TYPE_INFO, "Antika ve Efsanevi'de bu iþlemi yapamazsýnýz.")
			# return -1
	...
	def ActivateDragonSoulByExtern(self, deck):
		...
		self.RefreshEquipSlotWindow()
		self.ActivateEquipSlotWindow(deck)	#DS-Highlight-Fix
		if self.interface:
	...
	def DeactivateDragonSoul(self):
		self.isActivated = False
		self.activateButton.SetUp()
		self.DeactivateEquipSlotWindow()	#DS-Highlight-Fix
		if self.interface:
			self.interface.UseDSSButtonEffect(self.isActivated)
...
class DragonSoulRefineWindow(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		self.wndPercent.SetText("Szansa: |cFFf4be00{}%|r".format("0"))
	...
	def __CanRefineGrade(self, vnum):
		...
		if self.currentRecipe:
			...
			if self.currentRecipe:
				...
				self.wndPercent.SetText("Szansa: |cFFf4be00{}%|r".format(self.currentRecipe["percent"]))
				return True
			else:
	...
	def AppendStrengthPercent(self, strength = -1, vnum = -1):
		...
		if self.firstItem >= 0 and self.secondItem > 0:
			...
			self.wndPercent.SetText("Szansa: |cFFf4be00{}%|r".format(percent))
	...
	def __PressDoFastRefineStart(self):
		...
		# if self.wndDragonSoul.InventoryPageIndexDS() >=0 :
		...
		for i in xrange(player.DRAGON_SOUL_PAGE_SIZE):
			if not self.REFINE_TYPE_STRENGTH == self.currentRefineType and not self.REFINE_TYPE_STEP == self.currentRefineType:
	...
	def __CountRefineItem(self):
		...
		# if self.wndDragonSoul.InventoryPageIndexDS() >= 2:
			# chat.AppendChat(chat.CHAT_TYPE_INFO, "Antika ve Efsanevi'de bu iþlemi yapamazsýnýz.")
			# return 0
	...
	def Refresh(self):
		if self.REFINE_TYPE_STRENGTH == self.currentRefineType or self.REFINE_TYPE_STEP == self.currentRefineType:
