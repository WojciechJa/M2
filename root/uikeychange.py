...
class KeyChangeWindow(ui.ScriptWindow):
	...
	## KeySetting.txt ÆÄÀÏ ÀØ¾î ¿Í¼­ ¼ÂÆØ
	def LoadKeyInfo(self):
		...
		if count > 0:
			...
			## ¸¸¾à ½ºÅ©¸°¼¦ ½½·Ô¿¡ ´Ù¸¥ Å°°¡ ÀÖ´Ù¸é ØÊ±âØ­ ÇÏ°í ÀúÀå.
	...
	## =================== Å° ¼ÂÆØ¹× º¯°æ¿¡ ÇÊ¿äÇÑ Dick µé ¼ÂÆØ ºÎºØ ===================

	def __BuildKeyInfo(self, fileable):
		## ½ÇØ¦ UI Ç¥±âµÇ°í, ÀÛµ¿µÉ °Í Base
		KeyUiInfoDick = {}
		## ±âº»µ¿ÀÛ
		...
		KeyUiInfoDick[67] = app.DIK_P
		KeyUiInfoDick[68] = app.DIK_F5
		KeyUiInfoDick[69] = app.DIK_F6
		KeyUiInfoDick[70] = app.DIK_F7
		KeyUiInfoDick[71] = app.DIK_F8
		KeyUiInfoDick[72] = app.DIK_F9
		KeyUiInfoDick[73] = app.DIK_F11
		KeyUiInfoDick[74] = app.DIK_F12
		KeyUiInfoDick[75] = app.DIK_RETURN
		KeyUiInfoDick[76] = app.DIK_F10
		KeyUiInfoDick[77] = app.DIK_F1 + app.DIK_LALT + self.ADDKEYBUFFERALT
		KeyUiInfoDick[78] = app.DIK_F2 + app.DIK_LALT + self.ADDKEYBUFFERALT
		KeyUiInfoDick[79] = app.DIK_F3 + app.DIK_LALT + self.ADDKEYBUFFERALT
		KeyUiInfoDick[80] = app.DIK_F4 + app.DIK_LALT + self.ADDKEYBUFFERALT
		KeyUiInfoDick[81] = app.DIK_F5 + app.DIK_LALT + self.ADDKEYBUFFERALT
		KeyUiInfoDick[82] = app.DIK_F6 + app.DIK_LALT + self.ADDKEYBUFFERALT
		...
		## ØÊ±âØ­¿¡ ÇÊ¿äÇØ¼­ µû·Î ÇÏ³ª´õ ÀúÀåÇØµÒ.
		for i in xrange(self.KeySlotMax):
	...
	def __BuildKeyFunction(self):
		...
		KeyFunctionInfo[37] = player.KEY_TP_MAP
		## ½½·Ô ´ÜØàÅ°
		...
		KeyFunctionInfo[66] = player.KEY_OPEN_EXTENDED_INV
		KeyFunctionInfo[67] = player.KEY_OPEN_PET
		KeyFunctionInfo[68] = player.KEY_OPEN_DOPY
		KeyFunctionInfo[69] = player.KEY_OPEN_MARBLE
		KeyFunctionInfo[70] = player.KEY_OPEN_SAVE_LOCATION
		KeyFunctionInfo[71] = player.KEY_OPEN_RESP
		KeyFunctionInfo[72] = player.KEY_OPEN_BUFF
		KeyFunctionInfo[73] = player.KEY_OPEN_MISSION
		KeyFunctionInfo[74] = player.KEY_OPEN_COLLECTION
		KeyFunctionInfo[75] = player.KEY_RETURN
		KeyFunctionInfo[76] = player.KEY_OPEN_DUNGEONS
		KeyFunctionInfo[77] = player.KEY_CHANGECHANNEL1
		KeyFunctionInfo[78] = player.KEY_CHANGECHANNEL2
		KeyFunctionInfo[79] = player.KEY_CHANGECHANNEL3
		KeyFunctionInfo[80] = player.KEY_CHANGECHANNEL4
		KeyFunctionInfo[81] = player.KEY_CHANGECHANNEL5
		KeyFunctionInfo[82] = player.KEY_CHANGECHANNEL6
	...
	def __BuildKeyInfoText(self):
		...
		KeyTextDict[app.DIK_F11] = localeInfo.KEY_F11
		KeyTextDict[app.DIK_F12] = localeInfo.KEY_F12
		KeyTextDict[app.DIK_RETURN] = localeInfo.KEY_RETURN
	...
	def __BuildUsableKeyInfo(self):
		...
		UsableKeyDict[221] = app.DIK_F11
		UsableKeyDict[222] = app.DIK_F12
		UsableKeyDict[223] = app.DIK_RETURN

		self.UsableKeyDict = UsableKeyDict

	## =================== Å° ¼ÂÆØ¹Î º¯°æ¿¡ ÇÊ¿äÇÑ Dick µé ¼ÂÆØ ºÎºØ ===================
