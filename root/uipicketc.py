...
class PickETCDialog(ui.ScriptWindow):
	...
	def LoadDialog(self):
		...
		try:
			...
			# self.paczki = self.GetChild("paczki_value")
			# self.paczki_slot = self.GetChild("paczki_slot")
			# self.paczki_text = self.GetChild("text2")
			# self.paczki_val = self.GetChild("paczki_max_value")
		...
		self.checkBox.SetPosition(40, 57)
		...
		self.checkBox.SetTextInfo("Rozdziel wszystkie")
		self.checkBox.Show()
	...
	# def OnUpdate(self):
	# 	if self.doAll == True:
	# 		self.paczki_text.Hide()
	# 		self.paczki_slot.Hide()
	# 	else:
	# 		self.paczki_text.Show()
	# 		self.paczki_slot.Show()
	# 		pickvalue = self.pickValueEditLine.GetText()
	# 		obliczenie = (int(self.maxValue_global)/int(pickvalue))
	# 		self.paczki_val.SetText(" / "+str(obliczenie))

	def Open(self, maxValue, unitValue=1):
		self.maxValue_global = maxValue
