...
def FReturnInfo(func,index):
	...
	info = {
			...
			"vnum_material_7" : vnum_material_7,
			"count_material_7" : count_material_7,
			"gold" : gold,
			"punktyos" : punktyos,
			"cheque" : cheque,
			"percent" : percent,
...
class CubeRenewalItem(ui.Window):
	...
	def GetPunktyOs(self):
		return self.GetDates("punktyos")
	
	def GetCheque(self):
		return self.GetDates("cheque")
...
class CubeRenewalWindows(ui.ScriptWindow):
	...
	def __init__(self):
		...
		self.categoria = [
			[1,"ULEP",            localeInfo.CATEGORY_ULEP ],
			[2,"ALCHEMY",         localeInfo.CATEGORY_ALCHEMY ],
			[3,"WSPOMAGACZE",     localeInfo.CATEGORY_WSPOMAGACZE ],
			[4,"RIPER",           localeInfo.CATEGORY_RIPER ],
			[5,"WOJNY",           localeInfo.CATEGORY_WOJNY ],
			[6, "PASY",				localeInfo.CATEGORY_PASY ],
			[7, "LEGE_KD",			localeInfo.CATEGORY_LEGEKD ],
			[8, "Zwierzak",			localeInfo.CATEGORY_ZWIERZAK ],
			[9, "Owoce",			localeInfo.CATEGORY_OWOCE ],
			[10, "UM_PASYWNE",		localeInfo.CATEGORY_UM_PASYWNE ],
			[11, "PRZEPUSTKI",		localeInfo.CATEGORY_PRZEPUSTKI ],
			[12, "REKAWICE",		localeInfo.CATEGORY_REKAWICE ],
			[13,"MISJE",		localeInfo.CATEGORY_MISJE ],
			[14, "POMOCNIK",	localeInfo.CATEGORY_POMOCNIK ],
			[999,"WORLDARD",      localeInfo.CUBE_CATEGORY_ETC ]
		]
	...
	def LoadWindow(self):
		...
		try:
			...
			self.punkty_text = self.GetChild("punkty_text")
		...
		self.icons_items.SetSize((32*13),32*3)
		...
		for i in xrange(0,8):
			self.icons_items.AppendSlot(i+1,46*i+68,0,32,32*3)
	...
	def ClearElements(self):
		...
		self.yang_text.Hide()
		self.punkty_text.Hide()
		self.cheque_text.Hide()
	...
	def UpdateMaterialCount(self):
		...
		self.yang_text.SetText(localeInfo.NumberToGoldNotText(self.selectedItem.GetGold()*count))
		self.yang_text.Show()
		self.punkty_text.SetText(localeInfo.NumberToGoldNotText(self.selectedItem.GetPunktyOs()*count))
		self.punkty_text.Show()
		self.cheque_text.SetText(localeInfo.NumberToGoldNotText(self.selectedItem.GetCheque()*count))
		self.cheque_text.Show()

		if int(self.selectedItem.GetPunktyOs()*count-1) < player.GetPktOsiag():
			self.punkty_text.SetPackedFontColor(grp.GenerateColor(0.7607, 0.7607, 0.7607, 1.0))
		else:
			self.punkty_text.SetPackedFontColor(grp.GenerateColor(0.9, 0.4745, 0.4627, 1.0))

		if int(self.selectedItem.GetGold()*count-1) < player.GetElk():
		...
		if int(self.selectedItem.GetCheque()*count-1) < player.GetCheque():
			self.cheque_text.SetPackedFontColor(grp.GenerateColor(0.7607, 0.7607, 0.7607, 1.0))
		else:
			self.cheque_text.SetPackedFontColor(grp.GenerateColor(0.9, 0.4745, 0.4627, 1.0))
