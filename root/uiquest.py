...
class QuestDialog(ui.ScriptWindow):
	...
	def LoadDialog(self, skin):
		try:
			...
			PythonScriptLoader.LoadScriptFile(self, "arezzo/ui/questdialog.py")
	...
	def OnUpdate(self):
		...
		# AUTO_RESIZE_BOARD
			...
			if self.board:
				event.UpdateEventSet(self.descIndex, self.board.GetGlobalPosition()[0]+60+self.sx, -self.board.GetGlobalPosition()[1]-50-self.sy)
				event.SetEventSetWidth(self.descIndex, self.board.GetWidth()-70)
		elif self.skin:
			if self.board:
				event.UpdateEventSet(self.descIndex, self.board.GetGlobalPosition()[0]+60, -self.board.GetGlobalPosition()[1]-50)
				event.SetEventSetWidth(self.descIndex, self.board.GetWidth()-70)
	...
	def MakeNextButton(self, button_type):
		...
		b.SetSize(100,26)
		b.SetPosition(self.sx+self.board.GetWidth()/2-50,self.sy+yPos+40)
	...
	def MakeEachButton(self, i):
		if self.skin == 3:
			...
			button.SetPosition(self.sx + self.board.GetWidth()/2+((i*2)-1)*56-56, self.sy+(event.GetLineCount(self.descIndex))*16+20+5+40)
			button.SetText("a")
			...
			button.SetPosition(self.sx + self.board.GetWidth()/2-100,self.sy+(event.GetLineCount(self.descIndex)+i*2)*16+20+5+40)
			button.SetText("a")
	...
	def OnInsertImage(self, filename, underTitle, title, desc, index=0, total=1):
		...
		if underTitle:
			...
			event.InsertTextInline(self.descIndex, underTitle, (self.board.GetWidth() * (index + 1) / (total + 1)) - 40)
			if index != total - 1:
