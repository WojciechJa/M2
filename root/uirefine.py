...
class RefineDialogNew(ui.ScriptWindow):
	...
	def __LoadScript(self):
		...
		if app.ENABLE_REFINE_RENEWAL:
			...
			self.checkBox.SetTextInfo("Nie zamykaj okna")
			self.checkBox.Show()
	...
	def Open(self, targetItemPos, nextGradeItemVnum, cost, cost2, cost3, prob, type):
		...
		if self.percentage > 75:
			self.probText.SetText("Szansa na ulepszenie: |cFF84f05d"+str(self.percentage)+"%")
		elif self.percentage > 50 < 75:
			self.probText.SetText("Szansa na ulepszenie: |cFFffa12e"+str(self.percentage)+"%")
		else:
			self.probText.SetText("Szansa na ulepszenie: |cFFf25757"+str(self.percentage)+"%")
		# self.costText.SetText(localeInfo.REFINE_COST % (self.cost))
		# if self.cost > 0:
		self.costText.SetText("Cena: |cFFffc700" +('.'.join([ i-3<0 and str(cost)[:i] or str(cost)[i-3:i] for i in range(len(str(cost))%3, len(str(cost))+1, 3) if i ]))+" "+emoji.AppendEmoji("icon/emoji/money_icon.png"))
		self.costText2.Hide()
		self.costText3.Hide()
		if self.cost2 != 0:
			self.costText2.Show()
			self.costText2.SetText("|cFFb8b8b8" +('.'.join([ i-3<0 and str(cost2)[:i] or str(cost2)[i-3:i] for i in range(len(str(cost2))%3, len(str(cost2))+1, 3) if i ]))+" "+emoji.AppendEmoji("icon/emoji/cheque_icon.png"))
		if self.cost3 != 0:
			self.costText3.Show()
			self.costText3.SetText("|cFF85d455" +('.'.join([ i-3<0 and str(cost3)[:i] or str(cost3)[i-3:i] for i in range(len(str(cost3))%3, len(str(cost3))+1, 3) if i ]))+" "+emoji.AppendEmoji("d:/ymir work/ui/stone_point/stone_point.tga"))
	...
	if ENABLE_REFINE_ITEM_DESCRIPTION:
		...
		def SearchShop(self, slotIndex):
			...
			if not app.IsPressed(app.DIK_LCONTROL):
				if not app.IsPressed(app.DIK_LSHIFT):
					net.SendChatPacket("/search_in_shops 0 " + str(TOOLTIP_DATA[slotIndex])+ " 0 0 0")
		...
		def OverInItem(self, slotIndex):
			...
			if self.tooltipItem:
				...
				self.tooltipItem.AppendTextLine("ZnajdŸ przedmiot w sklepach.")
				self.tooltipItem.AppendSpace(5)
	...
	def AppendMaterial(self, vnum, count):
		...
		# search = ui.Button()
		# search.SetParent(thinBoard)
		# search.SetPosition(self.toolTip.GetWidth()-40, 6)
		# search.SetUpVisual("d:/ymir work/ui/search_item.png")
		# search.SetOverVisual("d:/ymir work/ui/search_item1.png")
		# search.SetDownVisual("d:/ymir work/ui/search_item2.png")
		# search.SetEvent(ui.__mem_func__(self.OverInItem()))
		# import chat
		# chat.AppendChat(chat.CHAT_TYPE_INFO, str(vnum))
		# search.Show()
		# self.children.append(search)
	...
	# def SearchShop(self, itemVnum):
	# 	net.SendChatPacket("/search_in_shops " + str(itemVnum))
