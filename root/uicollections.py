#-*- coding: iso-8859-1 -*-
import ui, app, net, localeInfo, uiToolTip, chat, player, item, emoji
...
BONUS_LIST = {
	0 : { 
		0: "Silny przeciwko Potworom +35%", 1: "Silny przeciwko Ludziom +10%", 2: "Œrednie Obra¿enia: 15%", 3: "Obra¿enia Koñcowe +5%"
	},
	1 : { 
		0: "Œrednie Obra¿enia: 10%"
	},
	2 : { 
		0: "Silny przeciwko Potworom +15%"
	},
	3 : { 
		0: "Silny przeciwko W³adcom +10%", 1: "Silny przeciwko Ludziom +25%"
	},
	4 : { 
		0: "Œrednie Obra¿enia: 10%", 1: "Silny przeciwko Potworom +10%"
	},
	5 : { 
		0: "Silny przeciwko Ludziom +30%", 1: "Œrednie Obra¿enia: 15%", 2: "Silny przeciwko Potworom +15%"
	},
}
CATEGORY = ["Bojownik", "Ulepiarz (I)", "Ulepiarz (II)", "Ulepiarz (III)", "Ulepiarz (IV)", "Ulepiarz (V)"]
...
class CollectionsWindow(ui.ScriptWindow):
	class CategoryButton(ui.RadioButton):
		...
		def GetProgress(self):
			if self.__NeedCount == 0 or self.__MyCount > self.__NeedCount: #Fix: Div/0 / Higher Percent Thank 100 -> didnt happend but chuj wie kto se tam co kliknie w bazie dla testÃ³w...
				return 100
	...
	def UpdateValue(self, collectionIdx, itemIndex, myCount):
		...
		if (it['mycount'] >= it['count']):
			...
			self.totalcount[int(itemIndex)].SetText("|cFF66d678Oddane")
		else:
	...
	def __SelectCategory(self, index):
		...
		self.__CategoryListBox.GetItem(self.__ActualCategory).SetUp()
		self.__ActualCategory = index
		self.__CategoryListBox.GetItem(self.__ActualCategory).Down()
	...
	def __RefreshItems(self):
		...
		for index in range(len(actualCategory.GetItems())):
			...
			if it['mycount'] >= it['count']:
				...
				self.totalcount[index].SetText("|cFF66d678Oddane")
			else:
