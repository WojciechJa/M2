...
bool DSManager::ExtractDragonHeart(LPCHARACTER ch, LPITEM pItem, LPITEM pExtractor)
{
	...
	if (pItem->IsEquipped())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "<Alchemia> Nie mo¿na tego zrobiæ.");
		return false;
	}
	...
	else
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "<Alchemia> Ekstrakcja zakoñczona pomyœlnie.");
		return true;
	}
}
...
bool DSManager::PullOut(LPCHARACTER ch, TItemPos DestCell, LPITEM& pItem, LPITEM pExtractor)
{
	...
	if (!IsValidCellForThisItem(pItem, DestCell) || ch->GetItem(DestCell)) // @fixme160 (added GetItem check)
	{
		...
		if (iEmptyCell < 0)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Niestety gracz ten nie ma miejsca w ekwipunku.");
			return false;
		}
		else
		{
...

	{
		...
		if (bSuccess)
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, "<Alchemia> Pomyœlnie wydobyto Smoczy Kamieñ.");
			pItem->AddToCharacter(ch, DestCell);
			return true;
		}
		else
		{
			...
			if (dwByProduct)
			{
				...
				if (pByProduct)
					ch->ChatPacket(CHAT_TYPE_INFO, "<Alchemia> Wydobywanie Smoczego Kamienia %s nie powiod³o siê.", pByProduct->GetName());
				else
					ch->ChatPacket(CHAT_TYPE_INFO, "[DEBUG] Linia: 481 | DragonSoul.cpp");
			}
			else
				ch->ChatPacket(CHAT_TYPE_INFO, "[DEBUG] Linia: 484 | DragonSoul.cpp");
		}
	}
...
bool DSManager::DoRefineGrade(LPCHARACTER ch, TItemPos (&aItemPoses)[DRAGON_SOUL_REFINE_GRID_SIZE])
{
	...
	if (ch->GetGold() < fee)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Nie masz wystarczaj¹cej iloœci Yang, aby ulepszyæ przedmiot.");
	...
	if (result_grade > grade_idx)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "<Alchemia> Ulepszanie zakoñczone sukcesem.");
	...
	else
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "<Alchemia> Nie uda³o siê poprawiæ przedmiotu.");
...
bool DSManager::DoRefineStep(LPCHARACTER ch, TItemPos (&aItemPoses)[DRAGON_SOUL_REFINE_GRID_SIZE])
{
	...
	else
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "<Alchemia> Ulepszanie zakoñczone niepowodzeniem.");
		SendRefineResultPacket(ch, DS_SUB_HEADER_REFINE_FAIL, TItemPos (pResultItem->GetWindow(), pResultItem->GetCell()));
		return false;
	}
}
...
bool DSManager::DoRefineStrength(LPCHARACTER ch, TItemPos (&aItemPoses)[DRAGON_SOUL_REFINE_GRID_SIZE])
{
	...
	if (fnumber(0.f, 100.f) <= fProb)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "<Alchemia> Wzmocnienie zakoñczone sukcesem!.");
	...
	else
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "<Alchemia> Wzmocnienie zakoñczone niepowodzeniem.");
		pDragonSoul->SetCount(pDragonSoul->GetCount() - 1);
		pRefineStone->SetCount(pRefineStone->GetCount() - 1);
		if (NULL != pResult)
			ch->AutoGiveItem(pResult, true);
