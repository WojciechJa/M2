...
{
#ifndef DISABLE_ANTI_FISHBOT
    DWORD iTries = number(1, 6);

    if (test_server)
        ch->ChatPacket(CHAT_TYPE_INFO, "(fishing) iTries = %d", iTries);

    char szEffectFile[MAX_EFFECT_FILE_NAME];
    snprintf(szEffectFile, sizeof(szEffectFile), "d:\\ymir work\\effect\\fishing\\%d.mse", iTries);

    TPacketGCSpecificEffect pe;
	pe.header = HEADER_GC_SPECIFIC_EFFECT;
	pe.vid = ch->GetVID();
	memcpy (pe.effect_file, szEffectFile, MAX_EFFECT_FILE_NAME);
    ch->GetDesc()->Packet(&pe, sizeof(pe));

    ch->SetQuestFlag("fishing.antibot", iTries);
#endif
	TPacketGCFishing p;
...
{
#ifndef DISABLE_ANTI_FISHBOT
    ch->SetQuestFlag("fishing.antibot", 0);
#endif
	TPacketGCFishing p;
	p.header = HEADER_GC_FISHING;
...
#ifndef DISABLE_ANTI_FISHBOT
EVENTFUNC(fishing_pull_rod_event)
{
    fishing_event_info * info = dynamic_cast<fishing_event_info *>( event->info );
    ...
    int iTries = ch->GetQuestFlag("fishing.antibot");

    if (iTries != 0)
    {
        ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("fishing_antibot_failure"));
...
LPEVENT CreateFishingPullEvent(LPEVENT pFishingEvent, LPCHARACTER ch)
{
    struct fishing_event_info* info = dynamic_cast<struct fishing_event_info*>(pFishingEvent->info);

    fishing_event_info* einfo = AllocEventInfo<fishing_event_info>();
    einfo->pid = info->pid;
    einfo->step	= info->step;
    einfo->hang_time = info->hang_time;
    einfo->fish_id = info->fish_id;

    event_cancel(&pFishingEvent);
    return event_create(fishing_pull_rod_event, einfo, PASSES_PER_SEC(1));
}
#endif
...
// 	if (ch->IsTitleActive(12))
// 		time = round(time*112/100);
// 	else if (ch->IsTitleActive(13))
// 		time = round(time*108/100);
// 	else if (ch->IsTitleActive(14))
// 		time = round(time*105/100);
// #endif
	TPacketGCFishing p;
	p.header	= HEADER_GC_FISHING;
