...
namespace quest
{
	...
	ALUA(_number)
	{
		...
		{
			lua_pushnumber(L, 0); // Zwraca 0, jeœli którykolwiek argument nie jest liczb¹
		}
		else if ((int)lua_tonumber(L, 1) > (int)lua_tonumber(L, 2))
		{
			lua_pushnumber(L, 0); // Zwraca 0, jeœli pierwszy argument jest wiêkszy od drugiego
		}
		else
		{
...
#ifdef ENABLE_ASLAN_BUFF_NPC_SYSTEM
				if (!pChar->IsPet() && !pChar->IsBuffNPC() && !pChar->IsHorse() && (true == pChar->IsMonster() || true == pChar->IsStone() || true == pChar->IsHorse()))
#else
