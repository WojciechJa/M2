...
void CDungeon::DecMember(LPCHARACTER ch)
{
	...
	if (m_set_pkCharacter.empty())
	{
		...
		event_cancel(&deadEvent);
#ifdef ENABLE_DUNGEON_RETURN
		deadEvent = event_create(dungeon_dead_event, info, PASSES_PER_SEC(DUNGEON_RETURN_TIME));
#else
...
#ifdef ENABLE_MINIMAP_DUNGEONINFO
void CDungeon::SetWarpAtEliminate(long time, long lMapIndex, int x, int y, const char* regen_file, DWORD bStage, DWORD bStageMax, const char* strNotice)
#else
...
{
	if (m_strMiniMapDungeonInfoNotice == nullptr)
	{
		sys_err("MiniMapDungeonInfoNotice is empty");
		return "";
	}

	return m_strMiniMapDungeonInfoNotice;
}
#endif
