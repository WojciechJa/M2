		...
		case QID_AUTH_LOGIN:
			{
				...
				else
				{
...

					{
						...
						str_to_number(aiPremiumTimes[PREMIUM_PICKUP], row[col++]);
					...
					{
#ifdef ENABLE_HWID_SYSTEM
						char szHwid[256], szCpuID[256], szHDDModel[256], szMachineGUID[256], szHDDSerial[256], szQueryHwid[256];
						...
						snprintf(szQueryHwid, sizeof(szQueryHwid), "UPDATE account SET cpu_id='%s', hdd_model='%s', machine_guid='%s', mac_addr='%s', hdd_serial='%s' WHERE id=%u", 
							szCpuID, szHDDModel, szMachineGUID, szHwid, szHDDSerial, dwID);
						std::unique_ptr<SQLMsg> msgHwid(DBManager::instance().DirectQuery(szQueryHwid));

						std::unique_ptr<SQLMsg> _hwid_msg(DBManager::instance().DirectQuery("SELECT mac_addr, hdd_serial, machine_guid FROM srv2_account.banned_list"));
						bool bReturn = true;
						if (!_hwid_msg->uiSQLErrno)
						{
							if (_hwid_msg->Get()->uiNumRows)
							{
								...
								{
									if (!strcmp(row_hwid[0], szHwid) || !strcmp(row_hwid[1], szHDDSerial) || !strcmp(row_hwid[2], szMachineGUID))
									{
										LoginFailure(d, "HWID");
										sys_log(0, "   HWID");
										M2_DELETE(pinfo);
						...
						bool bPin = false;
						std::unique_ptr<SQLMsg> _pinmsg(DBManager::instance().DirectQuery("SELECT pin FROM srv1_account.account WHERE login='%s'", pinfo->login));
		...
		case QID_DB_STRING:
			{
				...
				for (uint i = 0; i < pMsg->Get()->uiNumRows; ++i)
				{
					...
					if (row[0] && row[1])
					{
						m_map_dbstring.emplace(row[0], row[1]);
