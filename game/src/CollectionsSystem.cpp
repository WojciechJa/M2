...
bool CSystemCollections::LoadCategories() 
{
    std::unique_ptr<SQLMsg> msg(DBManager::instance().DirectQuery(
        "SELECT collection_index, name, reward_type0, reward_value0, reward_type1, reward_value1, reward_type2, reward_value2, reward_type3, reward_value3, reward_type4, reward_value4, percent, affect_id FROM srv1_player.collections_categories"));
...
{
	std::unique_ptr<SQLMsg> msg(DBManager::instance().DirectQuery("SELECT collectionIndex, itemIndex, itemVnum, itemCount FROM srv1_player.collections_items"));
	...
	while ((row = mysql_fetch_row(msg->Get()->pSQLResult)))
	{
		...
		{
			sys_err("CSystemCollections::LoadItems: cannot find collection index %d for an item: idx %d ivnum %d icount %d", collectionIndex, item.index, item.iVnum, item.iCount);
		}
	}
...
void CSystemCollections::AddItem(CHARACTER* ch, const BYTE collectionIndex, const BYTE itemIndex, bool isAll)
{
	...
	if (!ch->CanDoAction() || !ch->CanHandleItem()) // Change CanDoAction on CanWarp if u havent got this function.
	{
