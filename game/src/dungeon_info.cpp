...
#define OR else
		...
		OR DUNGEON_TOKEN("BOSS_DROP_ITEM")
		{
			sDungeonItem.dwVnum = iValue1;
			sDungeonItem.wCount = iValue2;
			sDungeonItem.dwPercentage = iValue3;
...
void CDungeonInfoManager::Ranking(LPCHARACTER pkCh, BYTE byIndex, BYTE byRankType)
{
	...
	if (pSDungeonData)
	{
		...
		std::unique_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("SELECT `player`.`name`, `player`.`level`, `%s` FROM `srv1_player`.`dungeon_ranking`"
			"LEFT JOIN `srv1_player`.`player` ON `pid` = `player`.`id` "
			"INNER JOIN `srv1_account`.`account` ON `account`.`id` = `player`.`account_id` "
...
void CDungeonInfoManager::SendInfo(LPCHARACTER pkCh, BOOL bReload)
{
	...
	for (size_t iIndex = 0; iIndex < s_vecDungeonProto.size(); ++iIndex)
	{
		...
		for (size_t iSlot = 0; iSlot < pSDungeonData->vecBossDropItem.size(); ++iSlot)
		{
			...
			GCPacket.sBossDropItem[iSlot].dwPercentage = pSDungeonData->vecBossDropItem[iSlot].dwPercentage;
		}
