...
void CHARACTER_MANAGER::CheckEventForDrop(LPCHARACTER pkChr, LPCHARACTER pkKiller, std::vector<LPITEM>& vec_item)
{
	...
	eventPtr = CheckEventIsActive(DUNGEON_CHEST_LOOT_EVENT, killerEmpire);
	if (eventPtr)
	{
		const int prob = number(1, 100);
		const int success_prob = eventPtr->value[3];
		...
		if (prob < success_prob)
		{
			...
			constexpr DWORD m_lbookItems[] = { 90058, 90059, 90060, 90061, 90062, 90063, 90064 };
			std::vector<LPITEM> m_cache;
			for (const auto& vItem : vec_item)
			{
				...
				for (const auto& missionBook : m_lbookItems)
				{
					...
					{
						if (pkKiller->FindAffect(AFFECT_MALL) && pkKiller->GetQuestFlag("VIPBONUS.SILVEREXPIRE") < 1)
							rewardItem = ITEM_MANAGER::Instance().CreateItem(itemVnum, chest_value2, 0, true);
						else if (pkKiller->GetQuestFlag("VIPBONUS.SILVEREXPIRE") > 1)
