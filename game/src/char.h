...
//		bool			IsPet() const		{ return m_bCharType == CHAR_TYPE_PET; }
		...
		long long		GetPolymorphPoint(BYTE idx) const;
	...
	public:
		...
		bool DropItem(TItemPos Cell, int bCount = 0);
#else
		bool DropItem(TItemPos Cell, BYTE bCount = 0);
#endif
		...
		void 			OpenChestItem(LPITEM item, WORD bCount = 1);
		int IsEmptyDragonSoulInventoryByGradeAndSize(EDragonSoulGradeTypes grade, BYTE size);
		bool GetItemFromSpecialItemGroup(DWORD dwGroupNum, std::vector<DWORD> &dwItemVnums, std::vector<DWORD> &dwItemCounts);
#endif
#ifdef ENABLE_INSTANT_INGOT_OPEN
		bool			OpenIngotItem(TItemPos pos, int bOpenCount = 1);
		bool			OpenIngotItem(LPITEM item, int bOpenCount = 1);
#endif
...
#ifdef __EXTENDED_ITEM_COUNT__
		bool			MoveItem(TItemPos pos, TItemPos change_pos, int num);
#else
...
#ifdef __EXTENDED_ITEM_COUNT__
		LPITEM			AutoGiveItem(DWORD dwItemVnum, int bCount = 1, int iRarePct = -1, bool bMsg = true);
...
#endif
#ifdef ENABLE_CHEQUE_SYSTEM
		long long				GetCheque() const		{ return m_points.cheque;	}
		void			SetCheque(long long cheque)	{ m_points.cheque = cheque;	}
		bool 			DropCheque(long long cheque);
		void			GiveCheque(long long iAmount);
#endif
#ifdef ENABLE_PUNKTY_OSIAGNIEC
...
#ifdef __EXTENDED_ITEM_COUNT__
		void			OpenMyShop(const char* c_pszSign, TShopItemTable* pTable, int bItemCount);
#else
		void			OpenMyShop(const char* c_pszSign, TShopItemTable* pTable, BYTE bItemCount);
#endif
	...
	public:
		...
		struct TBattleInfoLegend
		{
			...
			TBattleInfoLegend(DWORD dVID, const char* sName, int iLv, int iRc, int iEmp, long long iTot, int iAggr)
		...
		void				UpdateAlignment(long long iAmount);
...
#ifdef ENABLE_LRN_BATTLE_PASS
	public:
		void				SetBattlePass(CBattlePassManager* pBattlePass);
		CBattlePassManager*	GetBattlePass() const { return m_pkBattlePass; }
		
	private:
		CBattlePassManager* m_pkBattlePass;
#endif
	...
	public:
		void	OnAlignUpdate(long long lOldAlignment);
#endif
	...
	public:
		...
		bool				CanActItemShop();
		int*				GetPremium() { return &m_aiPremiumTimes[0]; }

	private:
		int					m_iLastItemShopTime;
#endif

#ifdef __TOMBOLA__
	public:
		EVENTINFO(tombola_reward_event_info)
		{
			LPCHARACTER ch;
			LPITEM item;

			tombola_reward_event_info()
				: ch(0)
				, item(0)
			{
			}
		...
		void					SetTombolaPrize(CTombola::TTombolaItem pkPrizeVec);
		CTombola::TTombolaItem	GetTombolaPrize();
		void					TombolaRewardEvent(int iSecond);
		bool					m_bHasWheelData;
		LPEVENT					m_pkTombolaRewardEvent;
#endif
	// @fixme188 BEGIN
	public:
	...
	public:
		...
		long long	GetAcceCombineCheque(long lGrade);

		BYTE	GetAcceCombineChance(long lGrade);

		void	SetAcceCombineMaterials(long lGrade);
	...
	public:
		...
		LPITEM			GetBuffWear(WORD bCell) const;
	...
	public:
		...
		const char*	GetHwid(BYTE bComponent);
#endif
	...
	public:
		CItem* FindActiveToggleItem(int32_t toggleType, CItem* pException = nullptr);
#endif
#ifdef __RENEWAL_MOUNT__
		bool			MountBlockMap();
#endif
