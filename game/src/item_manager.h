	...
	public:
		...
		int GetMultiIndex(std::vector <int> &idx_vec) const
		{
			...
			if (m_bType == PCT)
			{
				...
				//if (fnumber(0.f, 100.f) <= m_vecProbs[0])
				if (fnumber(0.0f, 10000.0f) <= m_vecProbs[0] * 100.0f)
				{
				...
				for (uint i = 1; i < m_vecProbs.size(); i++)
				{
					...
					//if (fnumber(0.f, 100.f) <= m_vecProbs[i] - m_vecProbs[i-1])
					float fDiff = (m_vecProbs[i] - m_vecProbs[i - 1]) * 10000.0f / 100.0f;
					if (fnumber(0.0f, 10000.0f) <= fDiff)
					{
		...
		float GetOneIndex() const
		{
			float n = fnumber(0.0f, m_vecProbs.back());
...
class CMobItemGroup
{
	public:
		struct SMobItemGroupInfo
		{
			...
			SMobItemGroupInfo(DWORD dwItemVnum, int iCount, float iRarePct)
		...
		void AddItem(DWORD dwItemVnum, int iCount, int iPartPct, float iRarePct)
		{
			if (!m_vecProbs.empty())
...
class CDropItemGroup
{
	struct SDropItemGroupInfo
	{
		...
		SDropItemGroupInfo(DWORD dwVnum, float dwPct, int iCount)
	...
	void AddItem(DWORD dwItemVnum, float dwPct, int iCount)
	{
...
class CLevelItemGroup
{
	struct SLevelItemGroupInfo
	{
		...
		SLevelItemGroupInfo(DWORD dwVnum, float dwPct, int iCount)
			: dwVNum(dwVnum), dwPct(dwPct), iCount(iCount)
		{ }
	};
...
#ifdef __EXTENDED_ITEM_COUNT__
// #ifdef ENABLE_DROP_INFO_PCT
	SDropInfoItem(DWORD vnum, int count, float percentage)
// #endif
#else
...
#endif
		std::tuple<BYTE, std::array<short, 1>> 	RandomDefaultAttribute();
