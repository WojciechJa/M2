...
CGuild::CGuild(TGuildCreateParameter & cp)
{
	...
	{
		strlcpy(m_data.grade_array[i].grade_name, LC_TEXT("±ÊµÂø¯"), sizeof(m_data.grade_array[i].grade_name));
...
{
	if (ch->IsAntyExp()) {
		ch->ChatPacket(CHAT_TYPE_INFO, "Nie moøesz oddawaÊ doúwiadczenia majπc w≥πczony AntyExp!");
		return false;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> ¡¶∞¯«œ∞Ì¿⁄ «œ¥¬ ∞Ê«Ëƒ°∞° ≥≤¿∫ ∞Ê«Ëƒ°∫∏¥Ÿ ∏πΩ¿¥œ¥Ÿ."));
		return false;
	}
...
void CGuild::DeleteComment(LPCHARACTER ch, DWORD comment_id)
{
	...
	if (pmsg->Get()->uiAffectedRows == 0 || pmsg->Get()->uiAffectedRows == (uint32_t)-1)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> ªË¡¶«“ ºˆ æ¯¥¬ ±€¿‘¥œ¥Ÿ."));
	else
...
void CGuild::UseSkill(DWORD dwVnum, LPCHARACTER ch, DWORD pid)
{
	...
	if (GetSP() < iNeededSP)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> øÎΩ≈∑¬¿Ã ∫Œ¡∑«’¥œ¥Ÿ. (%d, %d)"), GetSP(), iNeededSP);
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> ƒ≈∏¿”¿Ã ≥°≥™¡ˆ æ æ∆ ±ÊµÂ Ω∫≈≥¿ª ªÁøÎ«“ ºˆ æ¯Ω¿¥œ¥Ÿ."));
		return;
	}
	...
	if (test_server)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> %d Ω∫≈≥¿ª ªÁøÎ«‘ (%d, %d) to %u"), dwVnum, GetSP(), iNeededSP, pid);
		...
		case GUILD_SKILL_TELEPORT:
			...
			else
			{
				if (m_memberP2POnline.find(pid) != m_memberP2POnline.end())
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> ªÛ¥Î∞° %d √§≥Œø° ¿÷Ω¿¥œ¥Ÿ. («ˆ¿Á √§≥Œ %d)"), pcci->bChannel, g_bChannel);
					}
					else
					{
					...
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> ªÛ¥Î∞° ø¬∂Û¿Œ ªÛ≈¬∞° æ∆¥’¥œ¥Ÿ."));
			}
			break;
		...
		default:
			{
				if (!UnderAnyWar())
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> ±ÊµÂ Ω∫≈≥¿∫ ±ÊµÂ¿¸ ¡ﬂø°∏∏ ªÁøÎ«“ ºˆ ¿÷Ω¿¥œ¥Ÿ."));
					return;
				}
...
bool CGuild::ChargeSP(LPCHARACTER ch, int iSP)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> %u¿« øÎΩ≈∑¬¿ª »∏∫π«œø¥Ω¿¥œ¥Ÿ."), iSP);
	}
	return true;
}
...
void CGuild::RequestDepositMoney(LPCHARACTER ch, int iGold)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> ¿·Ω√»ƒø° ¿ÃøÎ«ÿ¡÷Ω Ω√ø¿"));
		return;
	}
...
void CGuild::RequestWithdrawMoney(LPCHARACTER ch, int iGold)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> ±ÊµÂ ±›∞Ìø°º± ±ÊµÂ¿Â∏∏ √‚±›«“ ºˆ ¿÷Ω¿¥œ¥Ÿ."));
		return;
	}
...
void CGuild::Invite( LPCHARACTER pchInviter, LPCHARACTER pchInvitee )
{
	...
	{
	    pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> ªÛ¥ÎπÊ¿Ã √ ¥Î Ω≈√ª¿ª πﬁ¿ª ºˆ æ¯¥¬ ªÛ≈¬¿‘¥œ¥Ÿ."));
	    return;
	}
	...
	{
		pchInviter->ChatPacket( CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> ªÛ¥ÎπÊ¿Ã ±ÊµÂ √ ¥Î ∞≈∫Œ ªÛ≈¬¿‘¥œ¥Ÿ.") );
		return;
	}
	else if ( !HasGradeAuth( GetMember( pchInviter->GetPlayerID() )->grade, GUILD_AUTH_ADD_MEMBER ) )
	{
		pchInviter->ChatPacket( CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> ±ÊµÂø¯¿ª √ ¥Î«“ ±««—¿Ã æ¯Ω¿¥œ¥Ÿ.") );
		return;
	}
	else if ( pchInvitee->GetEmpire() != pchInviter->GetEmpire() )
	{
		pchInviter->ChatPacket( CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> ¥Ÿ∏• ¡¶±π ªÁ∂˜¿ª ±ÊµÂø° √ ¥Î«“ ºˆ æ¯Ω¿¥œ¥Ÿ.") );
		return;
	}
	...
	switch ( errcode )
	{
		...
		case GERR_WITHDRAWPENALTY:
						pchInviter->ChatPacket( CHAT_TYPE_INFO,
								LC_TEXT("<±ÊµÂ> ≈ª≈«— »ƒ %d¿œ¿Ã ¡ˆ≥™¡ˆ æ ¿∫ ªÁ∂˜¿∫ ±ÊµÂø° √ ¥Î«“ ºˆ æ¯Ω¿¥œ¥Ÿ."),
								quest::CQuestManager::instance().GetEventFlag( "guild_withdraw_delay" ) );
						return;
		case GERR_COMMISSIONPENALTY:
						pchInviter->ChatPacket( CHAT_TYPE_INFO,
								LC_TEXT("<±ÊµÂ> ±ÊµÂ∏¶ «ÿªÍ«— ¡ˆ %d¿œ¿Ã ¡ˆ≥™¡ˆ æ ¿∫ ªÁ∂˜¿∫ ±ÊµÂø° √ ¥Î«“ ºˆ æ¯Ω¿¥œ¥Ÿ."),
								quest::CQuestManager::instance().GetEventFlag( "guild_disband_delay") );
						return;
		case GERR_ALREADYJOIN:	pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> ªÛ¥ÎπÊ¿Ã ¿ÃπÃ ¥Ÿ∏• ±ÊµÂø° º”«ÿ¿÷Ω¿¥œ¥Ÿ.")); return;
		case GERR_GUILDISFULL:	pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> √÷¥Î ±ÊµÂø¯ ºˆ∏¶ √ ∞˙«ﬂΩ¿¥œ¥Ÿ.")); return;
		case GERR_GUILD_IS_IN_WAR : pchInviter->ChatPacket( CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> «ˆ¿Á ±ÊµÂ∞° ¿¸¿Ô ¡ﬂ ¿‘¥œ¥Ÿ.") ); return;
		case GERR_INVITE_LIMIT : pchInviter->ChatPacket( CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> «ˆ¿Á Ω≈±‘ ∞°¿‘ ¡¶«— ªÛ≈¬ ¿‘¥œ¥Ÿ.") ); return;
	...
	switch ( errcode )
	{
		...
		case GERR_COMMISSIONPENALTY:
						pchInvitee->ChatPacket( CHAT_TYPE_INFO,
								...
								quest::CQuestManager::instance().GetEventFlag( "guild_disband_delay") );
						return;
		case GERR_ALREADYJOIN:	pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> ªÛ¥ÎπÊ¿Ã ¿ÃπÃ ¥Ÿ∏• ±ÊµÂø° º”«ÿ¿÷Ω¿¥œ¥Ÿ.")); return;
		case GERR_GUILDISFULL:	pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> √÷¥Î ±ÊµÂø¯ ºˆ∏¶ √ ∞˙«ﬂΩ¿¥œ¥Ÿ.")); return;
		case GERR_GUILD_IS_IN_WAR : pchInvitee->ChatPacket( CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> «ˆ¿Á ±ÊµÂ∞° ¿¸¿Ô ¡ﬂ ¿‘¥œ¥Ÿ.") ); return;
		case GERR_INVITE_LIMIT : pchInvitee->ChatPacket( CHAT_TYPE_INFO, LC_TEXT("<±ÊµÂ> «ˆ¿Á Ω≈±‘ ∞°¿‘ ¡¶«— ªÛ≈¬ ¿‘¥œ¥Ÿ.") ); return;
