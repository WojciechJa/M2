...
namespace quest
{
	...
	ALUA(affect_add_collect)
	{
		...
		int affect = (int) lua_tonumber(L, 1);
		long value = (long) lua_tonumber(L, 3);
		long duration = (long) lua_tonumber(L, 4);

		bool bOverride = (lua_isboolean(L, 5)) ? lua_toboolean(L, 5) : false;


		ch->AddAffect(affect, aApplyInfo[applyOn].bPointType, value, 0, duration, 0, bOverride);
