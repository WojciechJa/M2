...
#ifdef ENABLE_LRN_LOCATION_SYSTEM
	PlayerLocationManager::instance().LoadLocationData(ch);
#endif
#ifdef ENABLE_SWITCHBOT
...
// #endif
	...
	ch->TitleSendGeneralInfo();
#endif

#ifdef ENABLE_LRN_BATTLE_PASS
	if (auto pBattlePass = ch->GetBattlePass(); pBattlePass)
	{
		// pBattlePass->RefreshReward();
		pBattlePass->SendReward();
		pBattlePass->SendQuest();
		pBattlePass->SendLevel();
	}
#endif
	...
	if (ch->GetMapIndex() >= 10000)
	{
		if (CWarMapManager::instance().IsWarMap(ch->GetMapIndex()))
			...
			LPDUNGEON pkDungeon = CDungeonManager::instance().FindByMapIndex(ch->GetMapIndex());
			if (pkDungeon && pkDungeon->GetParty() && pkDungeon->GetParty() != ch->GetParty())
			{
				if (!ch->IsGM()) {
					sys_err("[%s] Tried to join another players dungeon", ch->GetName());
					ch->GoHome();
				}
			}
			else
	...
	DWORD pida = ch->GetPlayerID();
	if (pida != 0 && CHorseNameManager::instance().GetPetName(pida) == NULL)
	{
		db_clientdesc->DBPacket(HEADER_GD_REQ_PET_NAME, 0, &pida, sizeof(DWORD));
	}
#endif
...
#ifdef ENABLE_MOUNT_COSTUME_SYSTEM
	// if ((int)ch->GetQuestFlag("mount_system.lv") == 0)
	// {
	// 	ch->SetQuestFlag("mount_system.lv", 1);
	// }
...
#ifdef ENABLE_NEW_PET_SYSTEM
	if ((int)ch->GetQuestFlag("pet_system.lv") == 0 && (int)ch->GetQuestFlag("pet_system.points") == 0)
	{
		ch->SetQuestFlag("pet_system.lv", 1);
		ch->SetQuestFlag("pet_system.points", 1);
	}
	...
	if (ch->GetQuestFlag("costume_option.hide_stole") != 0)
		ch->SetStoleCostumeHidden(true);
	else
		ch->SetStoleCostumeHidden(false);
#endif
