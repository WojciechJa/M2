...
ACMD(do_item)
{
	...
	if (item)
	{
		...
		else if (item->IsSkillBook())
		{
			...
			{
				M2_DESTROY_ITEM(item);
				ch->ChatPacket(CHAT_TYPE_INFO, "Dodatkowy ekwipunek jest pelny.");
			}
		}
...
struct FuncPurge
{
	...
	void operator () (LPENTITY ent)
	{
		...
		if (!pkChr->IsOfflineShopNPC() && pkChr->IsNPC() && !pkChr->IsPet() && !pkChr->IsBuffNPC() && !pkChr->IsHorse())
	#else
		if (!pkChr->IsOfflineShopNPC() && pkChr->IsNPC() && !pkChr->IsPet() && !pkChr->IsBuffNPC() && !pkChr->IsHorse())
	#endif
#else
		if (pkChr->IsNPC() && !pkChr->IsPet() && pkChr->GetRider() == NULL && !pkChr->IsHorse())
#endif
		{
...
ACMD(do_drop)
{
    ...
    ch->ChatPacket(CHAT_TYPE_INFO, "|cFFf2eb6d############ [%dx - %s] ############", loopCount, pkAttacker->GetName());
    ...
    for (const auto& info : givenItems)
    {
        ...
        if (pItem)
        {
            ch->ChatPacket(CHAT_TYPE_INFO, "|cFFfffeed%s - |cFFf2eb6dx%d", pItem->GetName(), itemCount);
...
ACMD(do_state)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "RESIST:");
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "STONE:%3d%%", tch->GetPoint(POINT_ATTBONUS_STONE));
	ch->ChatPacket(CHAT_TYPE_INFO, "BOSS:%3d%%", tch->GetPoint(POINT_ATTBONUS_BOSS));
	ch->ChatPacket(CHAT_TYPE_INFO, "WLADCY:%3d%%", tch->GetPoint(POINT_ATTBONUS_WLADCA));
	ch->ChatPacket(CHAT_TYPE_INFO, "DUNGEON:%3d%%", tch->GetPoint(POINT_ATTBONUS_DUNGEON));
	ch->ChatPacket(CHAT_TYPE_INFO, "DMG_BONUS:%3d%%", tch->GetPoint(POINT_DMG_BONUS));
	ch->ChatPacket(CHAT_TYPE_INFO, "FINAL_DMG_BONUS:%3d%%", tch->GetPoint(POINT_FINAL_DMG_BONUS));

#ifdef ENABLE_HWID_SYSTEM
	ch->ChatPacket(CHAT_TYPE_INFO, "---");
	ch->ChatPacket(CHAT_TYPE_INFO, "HWID:");
	ch->ChatPacket(CHAT_TYPE_INFO, "   CPUID: %s", tch->GetHwid(0));
	ch->ChatPacket(CHAT_TYPE_INFO, "   HDD_MODEL: %s", tch->GetHwid(1));
	ch->ChatPacket(CHAT_TYPE_INFO, "   M_GUID: %s", tch->GetHwid(2));
	ch->ChatPacket(CHAT_TYPE_INFO, "   MACADDR: %s", tch->GetHwid(3));
	ch->ChatPacket(CHAT_TYPE_INFO, "   HDD_SERIAL: %s", tch->GetHwid(4));
#endif
		...
		case DoSetTypes::CHEQUE: // cheque
			{
				...
				tch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Cheque: ADD[%lld] TOTAL[%lld]"), cheque, tch->GetCheque());
			}
			break;
#endif
#ifdef ENABLE_PUNKTY_OSIAGNIEC
		case DoSetTypes::PKT_OSIAG:
			{
				long long pkt_osiag = 0;
				str_to_number(pkt_osiag, arg3);
				tch->PointChange(POINT_PKT_OSIAG, pkt_osiag, true);
			}
			break;
#endif
	}
...
ACMD(do_reload)
{
	...
	if (*arg1)
	{
		switch (LOWER(*arg1))
		{
			...
			case 'f':
				ch->ChatPacket(CHAT_TYPE_INFO, "Reloading fishing.");
				fishing::Initialize();
				break;
...
#ifdef ENABLE_CUBE_RENEWAL_WORLDARD
			case 'c':	// cube
				ch->ChatPacket(CHAT_TYPE_INFO, "Reloading cube.");
				Cube_init();
				break;
#endif
			...
			case 'i':
				...
				CItemShopManager::instance().ReloadItems(ch);
				break;
#endif

			case 'v':
				ch->ChatPacket(CHAT_TYPE_INFO, "Reload version..");
				P2P_MANAGER::Instance().Reload(eReloadTypes::RELOAD_VERSION);
				break;
			...
			default:
				...
				if (strstr(arg1, "drop"))
				{
					...
					ITEM_MANAGER::instance().ClearDropInfoCache();
					ch->ChatPacket(CHAT_TYPE_COMMAND, "DropInfoRefresh");
#endif
				}
...
ACMD(do_give_item)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Usage: give_item <nick>");
		return;
	}
	...
	{
		TPacketGCGiveItem pack;
		pack.bHeader = HEADER_GC_GIVE_ITEMS;
		ch->GetDesc()->Packet(&pack, sizeof(TPacketGCGiveItem));
		ch->ChatPacket(CHAT_TYPE_INFO, "log wys쿪ny na serwer.");
	}
}
