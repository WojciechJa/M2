...
namespace quest
{
	...
	ALUA(pc_hide_costume)
	{
		...
		if (bPartPos == 1)
			...
			ch->SetStoleCostumeHidden(hidePart);
		else
		{
	...
	ALUA(pc_is_skill_book_bonus_chance)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();

		lua_pushboolean(L, ch->FindAffect(AFFECT_SKILL_BOOK_BONUS) ? true : false);
		return 1;
	}

	ALUA(pc_remove_skill_book_bonus_chance)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		ch->RemoveAffect(AFFECT_SKILL_BOOK_BONUS);
		return 0;
	}
	...
	ALUA(pc_get_pkt_osiag)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		lua_pushnumber(L, ch->GetPktOsiag());
		return 1;
	}
	ALUA(pc_set_pkt_osiag)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (lua_isnumber(L, 1)){
			ch->PointChange(POINT_PKT_OSIAG, lua_tonumber(L, 1));
		}
		return 1;
	}
#endif
	...
	{
#ifdef ENABLE_LONG_LONG
		long long  cheque = (long long)lua_tonumber(L, -1);
#else
...
#ifdef ENABLE_LONG_LONG
			sys_err("QUEST wrong ChangeCheque %lld (now %lld)", cheque, ch->GetCheque());
#else
...
teleport_area:
	...
	ALUA(pc_get_special_ride_vnum)
	{
		...
		if (pChar)
		{
			...
			if (Unique1 && UNIQUE_GROUP_SPECIAL_RIDE == Unique1->GetSpecialGroup())
			{
				...
				#ifdef ENABLE_NEW_MOUNT_SYSTEM
				lua_pushnumber(L, Unique1->GetValue(1));
				#endif
				return 3;
			}
			...
			if (Unique2 && UNIQUE_GROUP_SPECIAL_RIDE == Unique2->GetSpecialGroup())
			{
				...
				#ifdef ENABLE_NEW_MOUNT_SYSTEM
				lua_pushnumber(L, Unique2->GetValue(1));
				#endif
				return 3;
			}
	...
	void RegisterPCFunctionTable()
	{
		luaL_reg pc_functions[] =
		{
...
#ifdef ENABLE_PUNKTY_OSIAGNIEC
			{ "set_pkt_osiag",		pc_set_pkt_osiag		},
			{ "get_pkt_osiag",		pc_get_pkt_osiag		},
#endif
			...
			{ "is_skill_book_bonus_chance", pc_is_skill_book_bonus_chance},
			{ "remove_skill_book_bonus_chance", pc_remove_skill_book_bonus_chance},
