		...
		else
		{
			...
			for (int k = 1; k < 1024; ++k) // @fixme148 256 -> 1024
			{
				...
				if (loader.GetTokenVector(buf, &pTok))
				{
					...
					sys_log(0,"        name %s count %d prob %f rare %d", name.c_str(), iCount, iProb, iRarePct);
		...
		else if (strType == "drop")
		{
			...
			for (int k = 1; k < 256; ++k)
			{
				...
				if (loader.GetTokenVector(buf, &pTok))
				{
					...
					float dwPct = (10000.0f * fPercent);

					sys_log(0,"        name %s pct %f count %d", name.c_str(), dwPct, iCount);
...
bool ITEM_MANAGER::GetApplyRandom(BYTE bIndex, int iLevel, BYTE& bApplyType, short& sApplyValue, BYTE& bPath)
{
	if (m_pApplyRandomTable == nullptr)
		return false;

	return m_pApplyRandomTable->GetApplyRandom(bIndex, iLevel, bApplyType, sApplyValue, bPath);
}

short ITEM_MANAGER::GetNextApplyRandomValue(BYTE bIndex, int iLevel, BYTE bPath, BYTE bApplyType)
{
	if (m_pApplyRandomTable == nullptr)
		return 0;

	return m_pApplyRandomTable->GetNextApplyRandomValue(bIndex, iLevel, bPath, bApplyType);
}
#endif

//martysama0134's 4e8ae61e0682ef69b602969d6cdf2805
