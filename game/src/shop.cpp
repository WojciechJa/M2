...
#ifdef ENABLE_SHOP_USE_CHEQUE
	long long dwCheque = r_item.cheque;
#endif
...
#ifdef ENABLE_SHOP_USE_CHEQUE
	if ((long long)dwPrice>0 && (long long)dwCheque > 0) // Yang-Cheque
	{
		if (ch->GetGold() < (long long)dwPrice || ch->GetCheque() < (long long)dwCheque)
			return SHOP_SUBHEADER_GC_NOT_ENOUGH_MONEY_CHEQUE;
	}
	else if ((long long)dwPrice>0 && (long long)dwCheque <= 0) // Yang
	{
		if (ch->GetGold() < (long long)dwPrice)
			return SHOP_SUBHEADER_GC_NOT_ENOUGH_MONEY;
	}
	else if ((long long)dwPrice<=0 && (long long)dwCheque > 0) // cheque
	{
		if (ch->GetCheque() < (long long)dwCheque)
			return SHOP_SUBHEADER_GC_NOT_ENOUGH_CHEQUE;
	}
#else
	if (ch->GetGold() < (int) dwPrice)
...
#ifdef ENABLE_PUNKTY_OSIAGNIEC
	if (m_IsPktOsiagShop)
		pack2.ispktosiagshop = m_IsPktOsiagShop;
#endif
