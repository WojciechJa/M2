...
bool DragonSoulTable::CheckRefineStepTables ()
{
	...
	else
	{
		for (size_t i = 0; i < m_vecDragonSoulTypes.size(); i++)
		{
			for (int j = 0; j < DRAGON_SOUL_STEP_MAX; j++) // DRAGON_SOUL_STEP_MAX-1 jezeli nie mozna mixowac wybornych
			{
				int need_count, fee;
				std::vector <float> vec_probs;
...
{
	if (step_idx > DRAGON_SOUL_STEP_MAX) // >= DRAGON_SOUL_STEP_MAX-1 jezeli nie mozna mixowac wybornych
	{
